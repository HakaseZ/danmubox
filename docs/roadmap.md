...
| 项 | 说明 | 触发 / 前置 |
|---|---|---|
| A22 剩余：PK / 活动期命令 | `RANK_CHANGED_V2` / `PK_INFO` / `WIDGET_BANNER` / `UNIVERSAL_EVENT_GIFT(_V2)` 的语义与是否携带用户可见内容 | 需 PK / 连麦 / 抽奖活动期房间的载荷样本（一次 40 秒观察里一条未现） |
| 连线礼物映射 | `UNIVERSAL_EVENT_GIFT_V2` → 礼物条目 | 同上。社区文档有字段表，但嵌套结构未实测，按项目规矩不得凭文档猜结构 |
| A26 房管表情包分类 | 房管专属表情包的 `identity` 语义 | 需一个真有房管专属包的房间（实测：即便账号在房间里是房管，被测房间也只有通用包） |
