...
| 合并计数 | 礼物行始终渲染 `×N`（连击折叠后的 `count`，未折叠时为 `×1`）；其余 `kind` 不出现 `×N` —— `count > 1` 只可能来自礼物连击（§8.4）。`×N` 是正文**行内**末尾一格：跟在最后一行文字后面，不另占一行、不另开一栏 |
...
| 同一条只出现一次 | 回填与实时是**上游两条路**，同一条可能两边都有（`gethistory` 与 WS，或同一帧里压缩子包与明文子包各一份）。第二份在**进总线之前**就被丢掉（`danmubox-core` 的 `MessageSink::publish_with`，按 `uid + ts + 正文 + 表情` 认同一性）：界面与 `history_query` 快照看到的必须是同一份事实。**只对 `danmaku` 去重** —— 礼物 / 互动允许上游反复推同一条（连击、榜单刷新）。这条去重与显示层的折叠互不相干：它砍的是「同一条被送了两遍」，真人重复发言每条各有各的上游 `ts`，照旧各占一行；礼物连击仍按 §8.4 折叠成 ×N |
...
同一个 `combo_id`（上游 `batch_combo_id`，契约 §5）且**相邻**的礼物折叠成一行：`count` 记折叠了几条、`amount` 累加为整串总额，正文行尾渲染 `礼物 ×N`。这是**礼物连击**的固有形态（刷屏本来就是同一个动作的重复），不是可关的设置。
...
| 其余 `kind` | **不折叠**，一条消息一行（早先的「相似消息合并」已删除 —— 沿革见 [`CHANGELOG.md`](../CHANGELOG.md) 的 Removed 段，台账见 [`requests.md`](requests.md) P49） |
