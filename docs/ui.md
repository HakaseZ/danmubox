...
| 长度提示 | 超过上游长度上限时，发送按钮禁用并显示计数；上限值见 `protocol.md` 附录 A |
...
| 弹幕列表 | 唯一生长区与滚动区 | 同左 |
| 弹出面板（表情 / 短语 / 筛选） | 文档流里的一块，向上展开，只挤压列表（不遮最新一条）；限高 `--panel-max-h` = 260px；另有 `--live-dot` = 8px（状态点**看得见的那颗点**的直径，元素盒仍是 `--sp-3` 12px、热区不缩）与 `--title-gap` = 2.5em（循环滚动的标题两份拷贝之间的间隙）。**表情面板是例外**：面板顶上去掉了标题与「关闭」，网格区与左侧轨道同高、都定死为**三行大表情格**（`--emote-grid-h`，两组同一个高度，§6.3），因此面板实际高度只有三行 + 上下内边距（宽屏 169.1px、窄屏 177.1px），两列各自滚 | **同一口径**：也在文档流里，只挤压列表、不遮最新一条；区别只在限高——窄屏用视口份额 `--panel-max-h-narrow` = **45vh**（844px 下 ≈ 380px），内容超出由**面板内部滚动**承担，不去吃列表空间。表情面板按**三行大表情格**定高（`--emote-grid-h`，比 45vh 更矮），顶上没有标题与「关闭」（再点一次「表情」或点输入区外面收起） |
...
**窄屏的面板也是「挤列表」**：两个视口的差别只剩**限高的算法** —— 宽屏固定 260px，窄屏用视口份额 45vh（≈380px），超出由面板内部滚动承担，列表仍剩三分之一以上。**短语面板与表情面板是这条里的例外**：两者都在自己内部再分「固定行 + 可滚区」——短语是「面板头 + 加一条」固定、芯片区滚（§6.2），表情是「面板头 + tab 轨道」固定、表情格滚（§6.3）。理由分别是「加短语那一行不许被顶走」与「切组的 tab 不许被滚走」，共同点是**这两样都是入口，不能因为内容变多就消失**。**账号对话框**是模态流程，不受这条约束，保持自底 sheet。
