...
**窄屏的面板也是「挤列表」**：两个视口的差别只剩**定高口径里的上下内边距**（宽屏 `--sp-2`、窄屏 `--sp-3`）与**限高**（宽屏固定 260px，窄屏用视口份额 45vh ≈ 380px）。三个弹出面板（表情 / 短语 / 筛选）在**两个视口都是定高**（`--panel-h` = 三行大表情格 + 面板上下内边距，见 §6.1）：面板整体不随内容伸缩，超出部分由**面板内部**的可滚区承担（短语是芯片区滚、表情是网格滚、筛选是面板自己滚），列表仍剩三分之一以上。**短语面板与表情面板是它们自己内部再分「固定行 + 可滚区」的那两个**：短语是「加一条」固定、芯片区滚（§6.2），表情是 tab 轨道 + 网格同高、都各自滚（§6.3）；共同点是**这两样都是入口，不能因为内容变多就消失**（加短语那一行、切组的 tab）。**账号对话框**是模态流程，不受这条约束，保持自底 sheet。
