...
| A32 | 历史条目的表情字段 | 回填的历史弹幕里，表情信息在顶层 `emoticon` 对象的哪些字段上 | 在弹幕活跃的在播房间上抓一次 `gethistory`，找带表情的条目比对 | **按接口文档实现、尚未观测**：顶部 `emoticon.{url, emoticon_unique, text, width, height, is_dynamic}`，以 `url` 非空判定为表情弹幕（实时弹幕的对应位置是 `info[0][13]`，两者布局不同）。采集时该接口正处于 A30 记录的空窗期，未能取到样本 | `history.rs` |
