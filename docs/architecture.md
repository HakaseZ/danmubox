...
| 电池余额的单位与刷新时机 | **已定（2026-09-11）**：上游只给金瓜子（`data.gold`），**电池 = gold / 100**（1 电池 = 0.1 元）；进入礼物相关界面时按需读取，不做后台轮询 | 已用真实凭据校对（`gold=15000` ↔ 15 元 ↔ 150 电池，两条口径一致，见 `protocol.md` A29） | 钱包端口维护者：上游若改口径，只需改 `wallet.rs` 的换算常量与 A29 记录 |
