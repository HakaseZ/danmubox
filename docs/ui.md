...
字段与取值以 [`contract.md`](contract.md) 为准；IPC 命令与事件形状见 [`ipc.md`](ipc.md)；上游协议与发送 / 举报的事实见 [`protocol.md`](protocol.md)（未实测项一律见其附录 A）；登录与凭据流程见 [`auth.md`](auth.md)。UI **不得**新增 `kind`、字段或偏好键。

**安全红线（契约 §4.1）**：`SESSDATA`、`bili_jct`、`DedeUserID` **不得**进日志、不得进前端明文、不得进仓库、不得进崩溃上报。前端只消费 `session_status` / `accounts_list` 的登录态与身份字段；界面**没有任何**凭据输入口，因此凭据值不会经 IPC 上行。
...
| 区块 | 内容 | 排序 / 规则 |
|---|---|---|
| **页头** | **一行**：左「弹幕框」标题（`--fs-8`），右**主题按钮**（`ui.theme` 三档：亮 / 暗 / 自动；**点一下循环一档**，图标随档变：日 / 月 / 日月） | 主题是**全局**的，因此控件放在主界面页头，而不是房间页的筛选面板里；形态是**按钮**，不是滑块、不是下拉（控件口径见 §8.3）。标题可缩、控件不缩，360px 宽也不换行、不溢出 |
| **账号区** | **只占一行**：当前身份（头像 + 昵称 + `uid`；未登录时只显示「游客态」）。**整行就是入口** —— 点这一块圆角长方形（键盘聚焦后 Enter / Space 等价）打开**账号管理对话框**（§2.2.1）；游客态同样可点，那时的作用就是去登录。**没有独立的「账号」按钮**，下拉、删除、新增、扫码、登出这些控件也不再内联在这一行 | 行内**没有别的可点元素**（整行只有一个入口，不会点错）；`role="button"` + `tabIndex=0` + `aria-label="账号管理"`，悬停加 `--hover-wash`、键盘焦点给强调色描边 |
| **添加房间** | 输入短号 / URL / 房间号 → `rooms_add` | 房间不存在时提示「房间不存在或短号无效」并保留输入 |
| **我的房间** | `rooms_list` 中手动添加的房间，带删除按钮（`rooms_remove`）。行内是「**主播名 · 直播间名**」（`anchor_uname` · `title`）+ 直播状态 + 连接状态，**不显示房间号** | 上游返回顺序 |
| **关注列表** | `follow_list` 全量（**含未开播**；未开播那一份由主站关注关系 + 批量房间接口取得，直播侧 `GetWebList` 只给在播，见 `protocol.md` A28），头像 + 主播名 + 直播标题 + 状态标签 + 最后开播时间（**不显示房间号、不显示关注分组名**） | 排序：`live_status == 1` 置顶 → **最近观看降序**（`ui.recent_watched`；没看过的不在这一档、排在看过的之后）→ `live_start_at` 降序 → `online` 降序 → `room_id` 升序。前端分页，每页 `FOLLOW_PAGE_SIZE = 30`，>1 页时显示上一页/下一页与页码。**未开播条目没有「最后开播时间」可显示**（上游不给，`live_start_at` 恒为 0，界面就不渲染那个元素），排序在这档落回 `online` / 房间号（需求 §2.6） |
...
- 两个字段的来源都是 `getH5InfoByRoom`：`data.anchor_info.base_info.uname` / `data.room_info.title`。
  `getRoomPlayInfo`（房间解析那一步）的响应里**没有**这两个键（`protocol.md` A41），
  夹具 `apps/desktop/ui/smoke/fixtures/room-{play,h5}-info.json`；解析在 `map_room_play_info`。

**关注项的排布按宽度分两种形态**（同一套 DOM，只换 grid 模板；需求 §2.11）：
...
- 两档都**不渲染房间号**；也没有「短号」这类房间号变体。
- 头像列**永远占位**（`--avatar`），上游没给 `face` 的条目不会因此把右半边挤到别处。
- 直播间标题（`title`）超长省略（不撑破行）；上游未给（空串）时**不渲染该元素**，不用占位符顶位；数据来自 `follow_list` 条目自带的 `title`，不为每个房间另调接口。空着的格由 grid 模板保证，不让后面的格子错位。
- 关注分组名（`group_name`）**当前不渲染**：行内布局只有四个槽位，分组名没有位置；当前也**不自行分组排序**。字段仍随 `FollowedRoom` 带出（来自直播关注接口之外的另一套上游，`protocol.md` A34）；分组视图未排期 → `roadmap.md` §2.3（需求 §2.6）。
...
- **自动加载**：应用启动时（会话就绪后，即 `session_status` 返回且已登录）自动调一次 `follow_list`，
  无需手动点刷新；失败同样走上面的错误提示并保留「刷新」按钮，不静默。登录/换号/新建/删除账号后同样重新拉一次。
- **列表自己会刷状态**（周期与三条闸门见 `contract.md` §4）：
  ...
  **③失败退避**（30 秒只属正常周期；失败后按 **60 → 120 → 240 秒**逐次加倍并封顶，成功即复位 —— `store.ts:528-531`、`:578-582`）。
  轮询不带任何「加载中」占位：**状态点在原地变色**，列表不重铺、不闪、不打断滚动位置。
  它**与后台保活无关** —— 后台该不该活着由 `keepalive.ts` 与连接自身决定，不由这一页的轮询驱动。
- 列表项展示头像（`face`）、主播名（`uname`）、直播间标题（`title`）、直播状态标签、最后开播时间（`live_start_at`，未知则不显示该段）；**房间号不展示**。

### 2.2.1 账号管理对话框（`accounts_list` / `account_*`，需求 §2.5）

点账号区那一行打开（**整行即入口**：行上没有独立的「账号」按钮；游客态也点得开，作用就是去登录）；同一时刻只开一个，`Esc` 或点背景关闭。**头部没有「关闭」按钮**——关就是 `Esc` 或点背景，两条路都走同一个 `onClose`（取消扫码 + 收起对话框）。打开时先重拉一次 `accounts_list`
（列表必须说当下的事实）。
...
| 区域 | 内容 | 规则 |
|---|---|---|
| 账号行 | 头像 + **昵称** + `uid` + 账号名 + 状态 + 操作按钮 | 一行一个账号。状态三选一：`已登录 · 当前`（强调色边框标出）/ `已登录` / `未登录`（凭据失效也落在这里，界面不猜原因）。**昵称与 uid 必须露出来**。**非当前账号整行可点**：点击 / Enter / Space = 切到它（`role="button"` + `tabIndex=0` + `aria-label`）；当前账号行不可点、不可聚焦。行内动作按钮一律 `stopPropagation`，点它们不触发切换 |
| 操作 | `重新登录` / `退出登录`（`logged_in` 时才有）/ `删除` | 具体见下表。**没有「切换」按钮** |
| `＋ 添加账号` | **默认入口**。点了直接出二维码，**不先起名字** | `account_qr_start()` 不带 `target`：扫完由后端按昵称自动命名（重名加后缀）并落盘，列表随即多一行且标为「已登录 · 当前」。**这条路永不覆盖任何已有凭据** |
...
`--sp-3`（12px，来自 `.accountDialog` 的 `gap`），**不**另加外边距。这两条在宽屏与窄屏（≤ 520px 的贴底 sheet）下都成立。
...
| 情况 | 表现 |
|---|---|
| 账号**已登录**且 `anchor_room` 给出了直播间 | 正常渲染：状态 + 标题行 + 分区选择 + 开播 / 下播行 |
| 该账号**没有开通直播间**（后端 `anchor_room` 返回 `null`） | **按钮不显示**（第 2 条）：界面按 `anchorRooms[name]` 为 `null` 直接判定，根本不给入口，不渲染半截管理区 |
| **游客态**（未登录） | **按钮不显示**：未登录账号不在 `anchorRooms` 映射里，不给入口（第 2 条） |
...
**ROOM_CHANGE 自动更新标题（issue202609241553 第 6 条）**：弹幕 WS 推来的 `ROOM_CHANGE`（只带 `data.room_id` 与 `data.title`，字段形态见 `protocol.md` A66，取自参照实现 `HakaseZ/BiliLiveWatcher`）经 `danmubox://room` 事件回到前端，`store.onRoom` 命中 `anchorRoom.room_id` 时**就地更新**标题输入框，**不必手动重读**。限制：本仓只在「连接着自己直播间」时才收得到这条（`cmd.rs` 只在命中已登记房间时才冒泡标题）——未连接则收不到，这是弹幕 WS 的固有约束，不是 bug（`ui.md` 与 `protocol.md` 均已写明）。
...
**五个面板同时最多开一个**（房管面板 / 表情 / 短语 / 筛选 / 独立礼物栏；礼物栏由 `ui.gift_panel` 决定存不存在，**默认就在场**，所以这条互斥在默认形态下是实打实的五个）：打开任一个都会收起另外四个（状态都在 `RoomView` 一处，输入区那三个是受控的）。收起某一个不影响别人 —— 「再点一次工具按钮收起」与「点面板 / 输入区外面收起」两条途径都成立。
...
| 房管面板 | 与弹出面板同层（输入区上方），由房间头 `⋯` 菜单开合（**该菜单项在「是房管或主播本人」时才存在**，§4.9）；展开时同样只挤压共享分区（§4.9）。**进到有房管权限的房间就把数据加载好**，打开时再静默重拉一次（§4.9） |
...
| 点击「刷新」（`rooms_reconnect`） | **保留**，不清空 | 重连 | 仍属同一次会话；会话已被「断开连接」结束时改成**重建一次会话**（缓冲从空开始），界面同口径换成**新会话**的快照，见下 |
| 返回列表页 / 关闭标签 | **销毁并清空** | 断开 | 会话结束 |
| 再次进入同一房间 | 全新会话，从空开始 | 新建 | 不复用上一次会话的缓冲 |

- 缓冲**按 `kind` 分档**（契约 §4.3）：弹幕 5000 条、礼物 2000 条、SC 500 条、大航海 200 条、互动/进场 300 条、系统通知 200 条（各由 `history.buffer_rows_*` 覆盖），每档超出丢自己的最旧；进程退出即丢。
  礼物那一档内部再按金额切低 10% / 中 40% / 高 50%（价高的留得多）。分档对界面是**透明的**：`history_query` 与实时事件给出的仍是同一批消息、同一个到达顺序，只是「先丢谁」变了 —— 进场消息的洪水因此不再把弹幕顶掉。
- 会话内允许向上回滚查看（`history_query`）；ac站无历史回放接口，不做跨会话历史，也没有向前翻取更早内容的入口。
- **「刷新」在两档下的界面口径不同，判据是 `rooms[].connected`**（= 外壳里还有没有这个房间的运行时）：
  - **会话还在**（连接中 / 退避中 / 已连接）→ 原地重连：`messages` 一步都不动，已收消息与滚动位置照旧（§3.2）。
  - **会话已结束**（点过「断开连接」）→ 等价重新进房（契约 §4.3）：界面必须**换成新会话的快照** —— 先摘掉上一个会话的行（只留本地待确认行），建连后整批落地一次 `history_query`，与「进房间」是同一条口径（`store` 的 `syncSessionMessages` 一处实现，两处共用）。不换的后果不是「顺序看着有点乱」，而是**新消息一条都进不来**：新会话的 `local_id` 从 1 重新编号，而界面末尾那个号还是**上一个会话的号**，`onMessage` 的单调判据把新消息全判成「不更新」丢掉 —— 房间看着已连接，弹幕再也不上屏。
  - 判据**不能**取 `status[room_id]`：长连接掉线、准备退避重连时推的也是 `disconnected`（`crates/danmubox-bili/src/ws.rs`），那一次会话还在、缓冲还在，按「重新进房」处理会白白清掉正在看的弹幕。
  - **机制级验证**：单测 `node --test src/session-messages.test.ts`（在 `apps/desktop/ui` 下）：会话重建后新会话的弹幕照旧上屏、同一条从回填与实时两条路来只画一行、会话还在时刷新不动列表、本地待确认行活过换代、快照没覆盖到的行不被抹掉。
...
| `FilterBar.tsx` | 筛选与显示偏好面板：kind 全集（其中「互动」那一项绑的是 `ui.interact_single_slot`，见 §4.8）、字号、时间戳、礼物类两枚开关；两块都是**两列勾选清单**（键清单与排布见 §8.5）。主题控件不在这里（§8.3） |
...
| `aggregate.test.ts` | 纯函数层的机制级验证（在 `apps/desktop/ui` 下跑 `node --test src/aggregate.test.ts`，10 条用例）：折叠门槛（两条逐条显示、第三条到齐才并成一行）、观众门槛（同一个人的三条重复逐条）、窗口是**滑动**的（基准是上一条，含「长蔓延」用例）、没有条数上限、头像列只画前 3 位、关掉 `ui.danmaku_aggregate` 原样返回入参、折叠不改入参、只有弹幕参与、本地乐观行与空正文不参与（§8.4 第二条） |
...
- **沉浸态排在第 1 级的最前**（§2.3.1，需求 §2.11）：沉浸态里房间头与房间标签条都不在场上（返回键、标签都够不着），
  第 1 级先退出沉浸，同一个手势再按一次才轮到面板 → 回列表。
...
- **原生侧的兜底**：页面返回 `false` 后，外壳先按 **webview 自己的历史**返回（`canGoBack()` → `goBack()`，
  `MainActivity` 的 `handleBackNavigation = false` 收过来一并承担），
  没有历史才 `finish()` 退出应用 —— 所以第 3 级就是「退出应用」（原生实现 `apps/desktop/src-tauri/gen/android/app/src/main/java/dev/kksk/danmubox/MainActivity.kt:183`）。
- **桌面端零影响**：这个函数在桌面端同样挂上（契约要求「存在」），但**没有任何调用方** —— 只有 Android 外壳调它。
- **不做**：predictive back 的**动画**（手势进行中页面跟随缩放 / 退出）不实现，只保证「这次返回被谁消费」这一层（见 `roadmap.md` §2.3）；
...
**房间头不再写连接状态文字**：没有「已连接（缓冲 N）」、没有 `verified` 徽标，
也不留占位。连接状态的可观察面是**房间标签页上的圆点**（§3.3）与 `⋯` 菜单；`error` 的退避详情只在标签页圆点与 `⋯` 菜单上下文里体现。
顶栏是**半透明的一层**（`--header-bg` 混本主题的表面色 + 一道自上而下的高光 + 轻微模糊），深浅两套自动成立。

会话时长不占头部（避免在窄窗口挤压标题）；标题排的 `live_status` 由房间头那枚状态点承载，列表页的 `live_status` 标签照旧（§2.2）。

### 3.2 「刷新连接」（手动重连，`⋯` 菜单第一项）

用途：长连接卡住、推流暂时中断、或长时间无新消息时**手动重连**（需求 §2.6）。命令为 `rooms_reconnect`（契约 §7）。
...
| 会话已被「断开连接」结束 | 当场**重建一次会话**（等价于重新进房：缓冲从空开始），界面**同口径换成新会话的快照**（先摘掉上一个会话的行、建连后整批落地一次 `history_query`，只留本地待确认行；§2.4）。不换的话新会话的 `local_id` 从 1 重新编号，新消息会被「只收更新的号」那条静默丢掉。断连之后这颗键不得是死键 —— 死键会逼人返回列表再进来 |
...
一行固定为 **`[头像][正文块：身份行（用户名 + 身份牌，右端：时间戳）/ 正文行][连击计数（仅礼物连击行）]`**。
行内**不挂任何按钮**：行菜单只在**右键**上（房间头那枚 `⋯` 是另一个元素，保留）。

**身份与正文是一个整体，头像是一列**。六条口径：

| 口径 | 规则 |
|---|---|
| 身份行在上、正文行在下 | 正文块是**上下两行**（参考图 `打开ac站继续观看.png` 的口径）：第一行「用户名 + 身份牌」，第二行正文**另起一行**、左起点与用户名对齐。两块都是块级，所以正文必然落在自己的行上（结构本身给的悬挂缩进，不需要 `padding-left` / 负 `text-indent`）。⚠ 正文块用 **`display: flow-root`** 而不是 `block`：块级大表情带一个负的 `margin-block`，普通块会与它**边距折叠**、把整个正文块上提 1.05px 骑到身份行上（冒烟实测 bodyTop 比身份行底边高 1.1px）。 |
| 正文拿到整行宽度 | 正文的可用宽度 = 行宽 − 头像列（时间戳在**身份行内**，不再从正文里扣那 8ch），**不被身份簇挤走**，两种视口都 ≥ 视口的 50%。`--identity-max-w`（14em / 窄屏 10em）与 `grid-template-columns` 随两列布局一并删除。冒烟按 `fixtureTextBodyKeepsHalfViewport`（正文 ≥ 视口一半）、`fixtureTextBodyFillsBlock`（正文吃满正文块）、`fixtureBodyAlignedWithName`（正文左边缘 = 用户名左边缘）断言 |
| 头像顶部对齐身份行、比它稍高 | 头像 = **1.25 × 行盒**（26.25px），身份行（用户名 + 身份牌那一行）的行盒 = 1 × 行盒（21px）。头像列的**顶边**与身份行顶边对齐（参考图口径，不是垂直居中）。行高由正文块的两行决定，所以放大头像**不会**把行撑高。冒烟按 `layoutAvatarTopAlignedWithIdentity`（顶边差 < 1.5px）、`rowAvatarTallerThanIdentity`（≥ 1.15 倍）、`rowBadgeNotFollowingAvatar`（牌不跟着放大）与 `layoutAvatarNotRowCentered`（反面对照）断言 |
...
| 表情与文字同流 | 表情图放在正文块（`data-testid="db-msg-body"`）**里面**，与文字同一个行盒：高度 `--emote` = 1.1 × 行盒，上下各给一个负 `margin-block` 把超出行盒的那 10% 扣掉（图照画那么大，行盒与逐行间距不变）。宽度按族给死：**横条族（通用 200×60）走 `.contentEmoteWide`，盒宽 = 10/3 × `--emote`**；方图族（房间 / 粉丝牌 162×162）仍是见方盒。`bulge_display` 的大表情（2 × `--emote`）是块级、独占一行。表情不另起一列、不与文字各站一个基线 |
...
| 尺度同源、头像单独一档 | 行盒高 `--row-line` 是唯一基准：**头像 = 1.25 × 行盒**、身份牌 = 0.9 × 行盒（`--badge-h` **不跟头像走**：复用 `var(--avatar)` 会让牌随头像一起放大）、表情 = 1.1 × 行盒（§9.2）。基准用 `@property --row-line { syntax: "<length>" }` 注册——自定义属性默认在**使用处**按元素自己的字号解析，徽标（`--fs-1` = 0.79em）会把「0.9 行盒」算成 14.9px 而不是 18.9px。冒烟按 `rowScaleCoherent`（1.25 / 0.9 / 1.1）与 `rowScaleFollowsFontSlider`（0.85 / 1 / 1.6 三档字号下比值不变）断言 |

| 单元格 | 规则 |
|---|---|
| 时间戳 | `ui.show_timestamp` 打开时渲染（默认关闭）；位置在**身份行的右端**（`margin-left: auto` 推到最右），定宽 `8ch`（`--time-col`）+ `font-variant-numeric: tabular-nums` + 右对齐，因此**逐行右边缘相同、纵向对齐**。**没有身份行的行**（`kind === "system"`、或既无昵称又无徽标）不丢时间：它在正文块的**首行**单独占一格、同样靠右，正文仍从下一行的左边缘开始。宽窄屏都不隐藏 |
| 头像 | `Message.face`；空串不渲染，加载失败退化为昵称首字符占位（§4.2）。对齐口径见 §4.2：**顶部与身份行对齐**。**礼物 / SC / 大航海也走这一列**：SC 有 `data.uinfo.base.face`、`SEND_GIFT_V2` 的顶层有 `face`（已补进 `Message`），而 **V1 礼物（`SEND_GIFT`）与大航海（`GUARD_BUY` / `USER_TOAST_MSG`）在协议字段表里没有头像字段** —— 那两类 `face` 是空串，界面**不画假图**（`protocol.md` §10.2 / §10.6） |
...
| `interact` | `--fg-dim`（`.kindInteract`，`app.module.css:1414`） | 不进弹幕列表，改由底部浮层槽位呈现（§4.8） | 弱化显示；列表里没有它的行，见 §4.8 |
...
表情弹幕（`Message.emote` 非空）**画图不画字**：此时正文就是表情名，只显示文字会被当成「表情没渲染」。图与文字同高对齐，`alt` 与 `title` 都填表情名——图加载不出来时浏览器回退显示 alt，不至于变成空白。**真实载荷还说明它「整条就是一张表情」**：实测的 11085 条真实 `DANMU_MSG` 里，35 个 `emoticon_unique` 与 `info[1]`（表情名）一一对应、尺寸**全部** 162×162——表情包弹幕没有可排版的用户正文，因此按整条画图处理。

**另一族是「正文里的文字表情」（`[dog]` 这类）**，路由与上一条**不同**（`protocol.md` §10.1.x / 附录 A42）：`info[0][13]` 是空槽位 `"{}"`，图只在 `extra.emots` 这个 map 里（键就是正文里那个 token）。后端**只在「正文整条恰好是一个 token」时**填 `Message.emote`，因此那一类照上面画图（实测盒 23.1×23.1px，图真实加载）；token 夹在别的字里时正文保持原文（`Message.emote` 是「整条画图」语义，设了会吞掉用户的话），**正文内的行内替换本实现没做**——那种行会原样显示 `点歌 大风吹 刘惜君[dog]`。夹具：`smoke/fixtures/danmaku-rows.json` 的 `emots`（整条）与 `emots-inline`（混排，取证用）。

**弹幕行里的表情图宽高一律给死 + `object-fit: contain`**（`.contentEmote` 见方盒 / `.contentEmoteWide` 横条宽盒 / `.contentEmoteBulge`）：上游 CDN 的表情是原图直出，尺寸各档不同（通用表情 200×60 的横条、房间 / 粉丝牌 162×162 的方图），只写 `height` 时宽度会按原图比例算出来。冒烟按 `rowInlineEmoteWideBox`（通用那条的盒高 = 1.1 × 行盒、宽高比 = 10:3）、`rowInlineEmoteTallerThanText`（画出来的高度 > 1em）与 `rowInlineEmoteFitsRow`（宽盒也不许撑破行）、`fixtureEmoteImgExplicitBox` / `fixtureEmoteImgInsideColumn`（真实夹具那条大表情弹幕）断言。

表情尺寸**不用固定像素、全部用 em**，因此跟着 `ui.font_scale` 一起缩放：

| 位置 | 尺寸 | 说明 |
|---|---|---|
| 弹幕正文里的表情 | 高度 `--emote` = `1.1 × --row-line`；宽度按族各给死：见方盒（房间 / 粉丝牌 162×162）与**宽盒** `10/3 × --emote`（通用 200×60 的横条） | 以**行盒高**为基准；`ui.font_scale = 1`（正文 14px、行盒 21px）时高 = 23.1px。超出行盒的 10% 由负 `margin-block` 从行盒里扣掉，所以一行表情不会把行高撑得忽高忽低。宽盒见 §4.1 |
| `bulge_display = true`（大表情） | `2 × --emote` + 4px 圆角 | 官方把这类表情画两倍大，见官方 `emoticon.bulge img { height: 40px }` |
| 表情面板里的缩略图 | 通用 `1.5em`；**非通用（本房间 / 粉丝牌 / 大航海）`2.2em`** | 非通用那几族本来是大图，缩成通用那么大看不清 |

**一切来自上游 CDN 的图片都必须在无 Referer 的前提下请求**（`index.html` 的 `<meta name="referrer" content="no-referrer">`）：该 CDN 有防盗链，非 ac站来源返回 403，浏览器再以 ORB 拦掉，表现为整屏问号且控制台毫无线索。
...
- 徽标顺序固定为 `[职衔槽][大航海槽][粉丝牌]`，**整组挂在昵称右侧**（参考图口径：用户名在前，蓝底白字的房间牌跟在后面）；徽标不是行首元素，也不占行首竖条。
- 昵称与徽标组属于**同一个身份行**（`data-testid="db-msg-identity"`，行内间距 `--sp-1`，顺序为「昵称 → 身份牌」；「回复标记」那一格已删除，见 §4.1 的 @ 高亮）：身份是「谁在说话」的一部分，它在正文的**上一行**（§4.1），不是与正文并列的一栏。
- **大航海槽只认 `Message.guard_level`（本房间的大航海等级）**：粉丝牌上的 `medal_guard_level` 是**牌子所属房间**的身份，只用于牌面样式，**不得**拿它兜底画舰长标——否则「戴着别的房间舰长牌的人」在本房间也会亮出舰长标（契约 §5 已把两者分层）。

**形状照官方聊天栏量得的值**：

| 项 | 官方 | 本界面 |
|---|---|---|
| 徽标形状 | 胶囊：`border-radius: 8px`、`height: 15px`、`padding: 2px 6px 2px 2px`、12px 白字 | **长方形**（参考图口径）：尺寸用 em（`height: var(--badge-h)` = 0.9 × 行盒 / `font-size: 0.79em` / `border-radius: 0.45em`，仍小于半高 0.675em，所以还是矩形不是胶囊）随 `ui.font_scale` 缩放，牌高**不跟头像走** |
...
**粉丝牌只在「亮着」时才画**（`Message.medal_lit` ← 上游 `user.medal.is_light`）：官方前端的弹幕行渲染分支就是这么判的（`if (F?.is_lighted) { 追加粉丝牌 }`，而 `is_lighted` 由 `medal.is_light` 派生 —— 官方前端产物取证，`protocol.md` A43）：**没点亮的牌官方不画**，上游连 `v2_medal_color_*` 都给灰（实测 `#919298*`）。`filtering.badgesFor` 因此把未点亮的牌按「没有牌」处理（`medalLevel` / `medalName` 归零），`hasBadges` 与牌面都随之不出现。
...
`medal_guard_level`（牌子**所属房间**的舰长标记，契约 §5）留给牌面样式。本界面**不用**它做视觉区分：官方对它的具体画法没有实测依据（口径：不编造未实测的上游行为，未实测项统一见 `protocol.md` 附录 A），而现有牌面（真彩色 + 等级格）已够辨识。`contract.md` §5 与 `protocol.md` A39 已写明它不得用于本房间的舰长标。

**头像**（需求 §2.1，`Message.face`）：

| 情况 | 渲染 |
|---|---|
| `face` 非空 | 圆形缩略图（`--avatar` = `1.25 × --row-line`，比身份行稍高，随字号缩放），`alt=""`、`title=昵称`（装饰性图片，昵称已在旁边） |
| `face` 为空串 | **不画图**（上游没给就没有头像），但**头像列照常占位**（同样 `--avatar` 宽的空列，`data-testid="db-msg-avatar-col"`）：否则这一行的身份行 / 正文会整体左移，与其它行对不齐 |
| 加载失败 | 退化成昵称首字符的圆形占位（居中大写，底色 `--bg-input`） |
| 独立礼物栏里（§5.3） | **同一个 `Avatar` 组件、同一档尺寸**：基准取行盒派生的 `--avatar` = 1.25 × 行盒（26.25px @ `ui.font_scale = 1`），与弹幕行逐项相同。空串同样不画图，而且**头像列照常占位**（与弹幕行同一口径，「逐行对齐」这条在礼物栏里成立）—— V1 礼物与大航海因此在那里没有图、但有一列空位 |

**头像的对齐口径：顶部与身份行对齐、比身份行稍高**（按参考图对齐）。做法：头像列的高度 = 头像盒高（`--avatar` = **1.25 × 行盒** = 26.25px），行容器里顶对齐，于是列顶 = 行的内容顶 = 身份行的行盒顶（21px）；头像比身份行高出那 25% 向下探进正文行的带宽，但它在自己的列里，不压正文，行高由正文块的两行决定，所以放大头像**不会**把行撑高（行高 ≈ 58px 是两行本身的结果）。冒烟按 `layoutAvatarTopAlignedWithIdentity`（头像列顶边 − 身份行顶边 < 1.5px）、`rowAvatarTallerThanIdentity`（头像 ≥ 身份行的 1.15 倍）、`rowScaleFollowsFontSlider`（0.85 / 1 / 1.6 三档字号下比值不变）与 `layoutAvatarNotRowCentered`（反面对照）两个视口都断言。
...
1. **发出去的弹幕 = 我用别的客户端看到的样子**—— 插入的那一帧就要带齐上游行会显示的一切（昵称 / 头像 / 粉丝牌与等级 / 大航海 / 房管 / 表情 / 间距），回播到达时再把**上游权威值**换进来：`ts` / 头像 / 牌面真彩色 / `upstream_id`（本地那套是界面按 `room_session` 与本人上一条上游行凑的近似）。**粉丝牌只有佩戴着才画**（`RoomSession.my_medal_worn` ← `data.medal.is_weared`）：`room_session` 的 `my_medal_level` 说的是**持有**这块牌 —— 实测某账号在某房间持有 Lv1 牌却没戴，照它画就会先多出一块「别人都看不到的牌」，回播到了再消失（`protocol.md` A43）。
2. **看不出中间有回播**—— 换字段时必须**保留同一行身份**：`local_id` 照抄本地那个负数，React key 因此不变 ⇒ DOM 节点不重建、样式逐项不变；而且是**原位替换**（期间可能有别人插进来，不许挪到末尾）。
3. **被 ban 的那条要留着** —— 上游明确拒绝时**不删行**：正文划线、行尾写上游给的原因，草稿也留着。

**乐观行不加任何待确认视觉** —— 点击后插进来的那一条与已确认行**渲染逐项相同**（不透明度 / 行、昵称、正文的字色 / 字号全等，也没有任何标记）；插入时**不设** `send_state`，只有「发送没成」才写上它。
...
「回执到之前不作任何预告」这条是可验的：冒烟的失败那一轮**先把 `chat_send` 的回执扣住**（`__holdSend`），此时断言那条本地行既没有标记（`sendFailRowNoMarkBeforeOutcome`）也没有弱化（`sendFailRowNoFadeBeforeOutcome`，`opacity` 逐位等于 `1`），`__releaseSend` 之后才断言标记出现。
...
**收包侧拿不到客户端关联 id**（收包 `extra` 的键里没有发送时用的 `replay_dmid`），所以对账只能靠上面这三件事。

**`echoedLocals` 侧表**：本地行对上之后**不消失**（口径 2 要求保留同一行身份），`local_id` 仍是负数，于是它会**一直**满足前三条 —— 同正文连发两条时，第二条的回播会被第一条再次吸走，第二条白等 8 秒被判「未确认」。因此对上过的本地行记进列表**之外**的这张侧表：行上一个字段都不写，退场只记在侧表里。`pendingSeq` 进程内单调递增、号不复用，因此键永不冲突；退房时随缓冲一起清空。

锚住的两件事：**命中之后不新增一行**（字段换上、`local_id` 不变，净条数不变 ⇒「只出现一条」是构造性的，不是靠事后去重），**也不重建节点**（React key 不变 ⇒ 看不出回播）。

几条既有行为不受影响：本地乐观行**不参与**礼物连击折叠（`toDisplayRows` 里先看 `local_id` 的负号 —— 乐观行插入时没有 `send_state`，只有这个前缀能一直认出它）；插入与回播换字段都照常走自动跟随（贴底时贴底、用户已上滚时不拽他下来）；历史回填、右键菜单、`@` 高亮、时间戳开关一律沿用原规则。

本地行用什么当 id：`local_id` 取**负数**（`-1`、`-2`…）。真实 `local_id` 由后端按会话单调分配、恒为正（契约 §5），因此两者永不碰撞 —— React key 不会重，「比末尾更新的才收」那条单调判定也不会被带偏。**回播命中时也不换成上游的号**（换成上游的号就等于换 key、重建节点）。

**被吞（`"f"` / `"k"`）没有单独的视觉**：`store.send` 对非 `ok` 一律走同一套「那条留在列表里、标成**被拒**（正文划线 + 行尾写原因）+ §6.5.1 浮动提示」，被吞 / 被禁言 / 频率限制不分档，原因就是行上与浮片上那句 `detail`（上游原话 + code）；判定来源见 `protocol.md` §11.2。

表情选择器的内容 = **接口给的包**，两路合起来：`emotes_list` 的按房间包（通用 / 本房间 / 粉丝牌 / 大航海）与 `emotes_owned` 的主站「我的表情」，按 `emoticon_unique` 去重（同名时 `emotes_list` 优先）。
**面板只显示上游下发的表情**（「从收到的弹幕里学表情」机制已删）；弹幕里的表情渲染消费的是后端给的 `Message.emote`（§4.1），与面板内容无关。

「我的表情」并进这一路是为了分组完整：它不在直播表情接口里，只能从 `emotes_owned` 取（§6.3 的「我的表情」组，`upower_` 家族）。加载时机固定为「进房间且会话就绪后拉一次」，失败不阻塞输入框，只在面板里给原因与重试（§6.3）。

### 4.5 行右键菜单

行内不挂一排动作按钮（需求 §2.11）。
**右键任意消息行**弹出菜单（触屏长按在 Chromium 内核上会派发 `contextmenu`，Safari/WKWebView 不可靠——将来若上 iOS 需要另补长按手势）：
...
| 禁言… | 自己是房管**或主播本人**（`RoomSession.is_admin` 或 uid == `anchor_uid`）且目标 `uid != 0` 且不是自己 | 弹**二次确认条**：对象昵称 + uid + 时长选择（本场直播 / 永久 / 1 / 6 / 24 小时）；确认后 `admin_mute`，见 §4.9 |
...
**房管三项必须做权限前置**：身份取自 `RoomSession.is_admin`（IPC `room_session`，契约 §7），**主播本人也算有权限**（uid == `anchor_uid` 且非 0）；两者都不是就置灰并写明原因——绝不做成「点了再看上游权限错误码」。
...
| 呈现 | **与实时弹幕完全一致**：同一套配色 / 字号 / 间距 / 时间戳，不弱化、不加分界提示 |
| 数据源 | 只来自上游回填，且只取 `data.room`（`protocol.md` A30）；本地发送记录**不进**消息列表 |
...
两个开关都来自契约 §8（一个管「互动看不看」、一个管「系统类看不看」），与关键字过滤相互独立（求值顺序见 §8.1）。

| 开关 | 偏好键 | 默认 | 行为 |
|---|---|---|---|
| 互动消息（浮层槽位） | `ui.interact_single_slot` | `true` | 互动/进场消息**一律不进弹幕列表**，改在弹幕区底部浮层显示**最新一条**，下一条到来时快速顶掉上一条，空闲 `INTERACT_SLOT_MS`（4s）后自动淡出；**弹幕区底部为它预留一段高度**，浮层落在预留区内、压不到任何一行弹幕。关掉则互动消息**完全不显示**（列表不画、浮层也不画），预留高度一并收回 |
| 系统类消息 | `filter.kinds` 里的 `system` 项 | 不在白名单（不显示） | 取消勾选即 `system` 行一律不渲染；勾上则与普通行同样式渲染 |

- **预留高度**（用户 2026-09-26：浮层「几乎一定会挡住最新的一条弹幕」）：开关打开时弹幕区容器 `.chatWrap` 带 `data-interact-slot="true"`，CSS 给它一段下内边距（= 槽位高度 `calc(var(--avatar) + 8px)` 上下各一道安全间距），浮层的 `bottom` 落在这一段里 —— 于是它压不到任何一行弹幕。留的是**外层容器**的 `padding-bottom`，滚动容器的 `scrollHeight` 与贴底判据一个字节都没动（冒烟 `38-title-slot.mjs` 的 `interactSlot*` 三条断言按这个几何验）。开关关掉时不留这段高度，互动消息也完全不显示。
- **互动消息共用单槽位**（`ui.interact_single_slot`，默认开，仿官方网页直播间「互动消息」条）：互动/进场消息**整类移出弹幕列表**——`filtering.toDisplayRows` 在开关打开时直接跳过 `kind === "interact"`，因此不管量多大都不会在弹幕区逐行堆叠、挤占空间。它们改由 `InteractSlot` 浮层（`data-testid="db-interact-slot"`）呈现：固定在弹幕区底部偏左的半透明胶囊，显示当前房间**最新一条**互动消息（头像 + 文案，文案规则与弹幕行同一枚 `interactText`：有 `content` 用 `content`、否则回落「<昵称> 进入直播间」）；下一条互动到达时，旧的那条向上滑出、新的自下而上滑入（过渡 `INTERACT_SLOT_REPLACE_MS` = 220ms），逐条接力顶替；超过 `INTERACT_SLOT_MS`（4s）没有新互动则整体淡出，下一条到达瞬间恢复。浮层 `pointer-events: none`，不遮挡弹幕交互。它同样只是显示层派生：消息始终留在会话缓冲里，关掉这枚开关即完全不显示。

**「不丢内容」的口径不变**：互动消息始终留在会话缓冲里（受契约 §4.3 的上限约束），浮层呈现与预留高度都只是显示层的派生 —— 开关拨回去，互动消息按同样的顺序回来。（旧的「互动消息自动消失」开关 `ui.interact_auto_hide` 已整条删除：单槽位这枚键已经把「看不看互动」说完，再留一枚叠加的开关只会让用户不知道该拨哪一枚。）
...
### 4.9 房管：禁言 / 黑名单 / 屏蔽词（需求 §2.10）

房管能力分两个入口：**行右键菜单**（对某个具体的人）与**房管面板**（名单与屏蔽词的增删查）。权限前置统一取自 `RoomSession.is_admin`（IPC `room_session`，契约 §7）：**拿不到身份 = 按无权限渲染**，绝不先放行再等服务端报错。**主播本人同样有房管权限**（登录 uid == 该房间的 `anchor_uid` 且非 0）：房管面板入口与消息右键的房管三项对他一样可用 —— 上游禁言记录里的 `admin_level`（0 = 主播、1 = 房管）本就把主播算在房管动作内（用户 2026-09-26 第 1 条）。入口判据因此是「`is_admin` **或**主播本人」，不是置灰、不是点开再看上游报错。

| 入口 | 位置 | 内容 |
|---|---|---|
| 对某个人的操作 | 行右键菜单（§4.5） | 「禁言…」「拉黑」「解除禁言」三项；非房管置灰并给出原因 |
| 房管面板 | 房间头 `⋯` 菜单 →「房管面板」（**是房管或主播本人才渲染这一项**；两者都不是、或身份没取到时菜单里没有它） | 三个 tab：禁言名单 / 黑名单 / 屏蔽词；每个 tab 一块列表 + 增删 |

**房管面板**（`data-testid="db-admin-panel"`）与输入区的弹出面板同处文档流（输入区上方、向上展开），展开时只挤压弹幕列表，不遮挡最新弹幕。**房管面板 / 表情 / 短语 / 筛选 / 独立礼物栏五者互斥**：同时最多开一个，打开任一个都会收起另外四个（需求 §2.10）。

**面板头部**只有 tab 轨道与**关闭**两样（需求 §2.10）——**没有身份提示、也没有「刷新」按钮**。关闭是**图标按钮**（自绘 X、没有文字，可访问名靠 `aria-label="关闭"` + `title`），与房间头那两枚是**同一个控件族**（`.ctlRound` + `.ctlIcon`，40 × 40 正圆；图标规范见 §3.1）。面板里不写身份提示（入口本身只对房管出现，见上一节）；打开面板时本来就静默重取一次，读取失败因此**只能靠面板里的错误条**反馈（不能静默）。

**面板自上而下是三块，两排都贴顶**（需求 §2.10）：
...
- **第 2 排**：全选 / 取消全选（`db-admin-select-all`）、「已选 N 项」、「该 tab 的批量动作」（`db-admin-batch-action`），三者横向一排。
- **主操作按钮在批量模式下留在第 1 排**（不禁用、也不换成「退出批量」）：第 2 排是「多一排」而不是「换一排」，第 1 排因此在两种模式下都不跳位；退出批量的入口就是那枚按下的图标钮（`aria-pressed="true"`，再点一次回到原样）。
- 确认条（`db-admin-confirm`）不在这三块里：它仍挂在输入区上方（`RoomView` 的 `.adminConfirm`）。
...
- **批量**：**第 1 排末尾**的批量图标钮（`data-testid="db-admin-batch"`，`aria-pressed`，打开时从透明底换成强调色填充；图标是「清单 + 勾」——两行「勾 + 名字」，正是打开批量后名单的样子，规范同 §3.1）打开后**点行即选中**（行高亮 = 已选中，**不挂复选框**；键盘 `Tab` 聚焦后 Enter / Space 同样切换），第 1 排**正下方**升起第 2 排（`data-testid="db-admin-batch-bar"`）：该 tab 的「全选」/「取消全选」（`db-admin-select-all`）、「已选 N 项」、该 tab 的批量动作（`db-admin-batch-action`）。批量只作用当前 tab，**换 tab / 关批量即清空勾选**；勾选数按**当前列表**算——写成功后重读三块，已经不在名单里的勾选自动落下去，「已选 N 项」与按下的对象因此永远一致。
- **数据预载与刷新（无手动刷新按钮）**：**连上有房管权限的房间就把三块加载好**（`RoomView` 里依赖 `[canAdmin, room.room_id]` 的 effect——身份是异步到的，`isAdmin` 必须在依赖里，否则进房那一帧按无权限渲染、这一批数据永远不拉），打开面板时再**静默重拉一次**（无 spinner、无按钮）保证新鲜。**面板展开期间按 5 分钟周期静默轮询三块**（原 60 秒周期已废弃：房管面板不是高频功能，无差别轮询既招风控、又无谓打上游）：链条式调度（不重叠）、不可见整拍跳过、失败按 5 → 10 → 20 → 40 分钟封顶退避、成功复位，收起面板即停，落地前复核 `activeRoomId` 仍是它（防串房）。写操作成功后自动重读三块（以远端为准，不在本地猜上游怎么变）。
...
- **名单一行两个**：名单是**两列等宽网格**（行高 40px，窄屏 360 仍是两列），不再是「每行一项」的竖排；批量模式下**点行即选中**（行高亮 = 已选中，**不挂复选框**，键盘 `Tab` 聚焦后 Enter / Space 同样切换），右键菜单承载单点动作（与消息行同口径）。每行**只显示一个昵称**：跑马灯的第二份拷贝默认隐藏，只有真的放不下（`data-scroll`）时才出现 —— 短昵称因此不会再显示两遍（用户 2026-09-26 第 2、3、4 条）。
...
### 4.10 选中态：整行一层底色（需求 §2.11）

**底色的来源**：默认那层绿底是**浏览器自己的选区高亮**（`::selection`）——它贴着**字形**画：头像那一列是空的，于是底色被头像卡住，两侧也到不了界面边缘。

**把「底色画在哪」从字形搬到整行**（`MessageRow.tsx` + `app.module.css` 的 `.row`）：
...
**这一栏与弹幕区上下分区**（见 §5.4），
并且**栏内的行就是弹幕行** —— 行 = `MessageRow`（`scope="gift"`）、
列表 = `MessageList`（同一份虚拟列表 / 跟随 / 贴底 / 「回到最新」实现，两处各持自己的滚动位置）。
它**不在输入区下方**（位置 = §5.4 的共享分区）。默认形态（弹幕在上、礼物在下、礼物栏折叠着）：
...
| 项 | 规则 |
|---|---|
| 位置 | **与弹幕区共享一块上下分区，全宽**；`ui.gift_panel` 为真时出现（**默认就开**，所以默认形态下弹幕区是「弹幕 + 礼物 + SC + 大航海」双份呈现）。上下顺序、栏高比例、分割条与长按换位见 §5.4 |
...
| 总计条与筛选条（取代原折叠头） | 礼物栏展开后是**上中下三段**：`总计条`（贴在**屏幕中心那一端**、兼作拖动热区、`data-pane-head`）、`列表`、`筛选条`（贴在**远离中心那一端**）。**折叠态只剩总计条**，列表与筛选条都收起。总计条文案 = `礼物 N条 ¥X`（例「礼物 12条 ¥1,168.7」）；开启筛选后只给条数、不给金额（金额已分列在下面的筛选条），即 `礼物 N条`。右端是**展开 / 收起图标**（就是 §3.1 房间头返回键那枚 `<` 箭头，按 `data-dir` 旋转 90°，`aria-label` 随态翻转）—— 原「折叠头」被它吸收，展开态不再另有一条折叠头。**收起有两条路**：点这枚图标，或**拖 / 方向键微调分割线把份额压到下限（0.1）以下**（§5.4），不必非得点箭头。筛选条是三枚芯片 `[<icon> ¥<金额>]`，分别画礼物 / SC / 大航海三族当场的金额合计；点芯片切换 `ui.gift_pane_kinds` 并集筛选（不选 = 全显示，选多项 = 并集，契约 §8），纯派生、不改缓冲。**三格的数字按未筛选口径**（本场该族有数据就常驻，选中与否不影响它显示）—— 改前从**筛后**那一份统计，未选中的两格条数归零、被 `count > 0` 滤掉而**整个消失**，再也点不回来（用户 2026-09-26 报的「互斥」）；**总计条才是先筛选后汇总**。三族图标取自 `public/icons/`（`gift.svg` / `sc.png` / `guard.png`） |
...
这几枚开关**都只是显示层的派生**：原始消息始终留在会话缓冲里，关掉开关即复原。

需求 §2.7。

**两个区域是哪两处**：**弹幕区**（`.chatWrap` / `MessageList` 的 `scope="chat"`，行钩子
`db-msg-row`）与**礼物栏**（`db-pane-gift`，行钩子 `db-gift-row`）—— 共享上下分区的两栏
（§5.4），礼物类消息按 `ui.gift_in_danmaku` / `ui.gift_panel` 分别落进这两栏，**默认两处都有**。
判定与实现两处**共用同一份**（`filtering.isCheapGift` / `collapseCheapGiftRows`），不存在「哪一栏
算低价」的第二套口径：
...
| 不丢内容（硬口径） | 剔除 / 折叠 / 隐藏（`ui.gift_panel`、`ui.gift_in_danmaku`）/ 互动的浮层呈现（§4.8）**都只改显示**：原始消息一直留在会话缓冲里（只受契约 §4.3 的上限约束），这些开关**不得**让任何一条提前消失。`splitGiftRows` / `giftStatRows` / `toDisplayRows` 全是纯派生（不改入参），关掉开关下一次重算即**逐条复原** |
...
| 机制级验证 | 单测 `node --test src/filtering.test.ts`（`apps/desktop/ui`）：两个区域各折一次、关掉即逐条按原序原对象回来、剔除只改统计、边界四项、互动「开 = 浮层呈现 / 关 = 完全不显示」两种状态都不动缓冲、纯派生不改入参 |
| 冒烟按 | `cheapGift*` 一组：两枚开关默认关且可持久化（点开 → 翻偏好 → 再点回）、折叠开启后礼物栏**条目数真的变少**且合并行的 `×N` / 金额是整桶合计、剔除开启后**汇总金额与总条数真的变少**而礼物栏条目数不变、四个边界值各自归属、SC 与大航海在这两枚开关四种组合下都不变。`switchScope*` 一组：**两个区域各量一次**折叠（弹幕区的桶行带 `×2`、礼物栏条目 3 → 2）、关掉后两处逐条复原（顺序 / 条数 / 金额 / 统计串）、剔除只动统计不动任何一处的行、拨掉「互动」那一项后列表与浮层都不出现 |
...
输入区自上而下：弹出面板（若打开，见 §6.3）→ 引用条（有回复上下文时出现，带「取消」）→ 文本输入框 → 工具行（左：表情 / 短语 / 筛选 三个面板入口；右：字数计数 `已用/上限` + 发送）→ 发送结果行（**只在需要操作时**才渲染，见 §6.5）。

**三个弹出面板共用同一副骨架**：都**不带标题、不带关闭按钮**，收起只有两条路（再点一次那个工具按钮 / 点面板与输入区之外，见 §6.3「关面板」）；高度一律 = `--panel-h`（**三行大表情格 + 面板上下内边距 + 1px 上边框**，与表情面板同高），**不随内容伸缩** —— 短语面板的超额内容在芯片区滚（§6.2），筛选面板在面板内部滚（§8.5）。三个面板的字号口径也统一（都吃 body 的基准字号，**不跟** `ui.font_scale`；面板高度用 em 令牌算，因此是常数）。
...
| 功能 | 规则 |
|---|---|
| 自定义短语 | 新增 / 改名 / 删除，存 `composer.phrases`（字符串数组）。**新增**用面板**首行**（面板没有标题行，`data-testid="db-phrase-add"`：输入框占满除「添加」按钮外的整行 —— 「添加」是强调色填充的按钮，与芯片不同形；回车或「添加」都提交）；**改名与删除**在短语上**右键**，菜单两项「编辑 / 删除」，编辑是就地变成输入框（占满整行，Enter 提交、Esc 取消） |
| 加一条那一行不许被顶走 | 短语面板自上而下只有两段：**「加一条」（固定行）** → 芯片区（`overflow-y: auto`）。芯片再多也只让**芯片区**滚动，加一条那一行永远在场。面板高度是定值（`--panel-h`，与表情面板同高），因此芯片多少都不改面板高度（§6.1）。冒烟按 `phraseAddRowInsidePanel`（在面板内）、`phraseAddRowStaysPut`（相对面板顶的偏移不变）、`phraseAddRowWideEnough`（≥ 120px）与 `phraseChatInputStillVisible`（聊天输入框仍完整落在视口里、被面板顶到下面而不是被盖住）断言 |
| 芯片排布 | 芯片区是一组**等宽列**（`grid-template-columns: repeat(auto-fill, minmax(7.5em, 1fr))`，列数随宽度自适应）：竖屏 360px 下是 3 列，宽屏一行铺满；芯片文字左对齐、长短语照常换行（**不截断**自建短语的内容）；空态那句提示占整行 |
| 重复与空值 | 添加或改名撞上已有短语、或内容为空时**给出提示**（「这条短语已经有了」/「短语不能为空」），不静默丢弃 |
| 来源 | 只有自己建的短语（存 `composer.phrases`）：面板里**没有**内置短语 / 颜文字，打开面板时列表为空就是空 |
...
按**当前房间身份**加载（`RoomSession`，契约 §5）：`my_medal_level` / `my_medal_name` / `my_guard_level` / `is_admin`。面板结构：主体两列 ——**左侧竖向 tab 轨道**（分组，`data-testid="db-emote-tabs"`，每个 tab 一个 `db-emote-tab`）+ **右侧表情网格**（`db-emote-group`，格子里一个 `db-emote-item`）。**面板里没有搜索框**（冒烟 `panelNoSearch` 断言面板内 `input` 数为 0）：找表情靠分组 tab，没有「按名字子串过滤」那一层。

**一屏只画一组**：点 tab 切组、切组回到格子顶部；tab 只列**当前有内容的组**（派生值，不留在 state 里）。

**网格区高度 = 三行大表情格，两组同一个高度**：**面板那一层**（三个面板共用的那条规则）算好 `--emote-size-sm = 1.5 × --fs-6` 与 `--emote-size-big = 2.2 × --fs-7`（**两者都注册成 `@property` 的 `<length>`**，在面板那一层就算成 px 继承下去）、`--emote-row-h-big = --emote-size-big + 2 × --sp-1`（图片盒 + 格子上下内边距）、`--emote-grid-h = 3 × --emote-row-h-big + 2 × --sp-1`（三行 + 两道行距）。网格与左侧 tab 轨道都用这一个高度，**与当前是哪一组无关**：通用组看到的也是三行大格，换组时面板不会忽高忽低。

**通用组也按大格算**：高度是「大表情那一档」的口径 —— 大表情必须**完整看到 3 行**，而通用组的格子比它矮，按同一高度只会看到更多行、不会切掉半行。冒烟按 `panelEmoteGridThreeBigRows`（网格 = 3 × 大格 + 2 × 行距）、`panelEmoteGridSameHeightForBothGroups`（通用组与大表情组的网格高相等）、`panelEmoteHeightIsGridPlusPadding` 断言。
...
**分组归属与「当前身份能不能用」都由接口侧判定**，界面不自己算：拿到的 `package_kind` 出 tab，拿到 `Emote.locked` 就置灰。**面板里的每一格都来自上游下发**（`emotes_list` + `emotes_owned`）——界面不从收到的弹幕里补表情。上游**不按权限过滤**，所以「无权限」在界面上是**置灰列出来**，不是消失。
...
| 尺寸 | 通用 `1.5em`、**非通用（我的表情 / 本房间 / 粉丝牌 / 大航海）`2.2 × --fs-7`**（= `--emote-size-big`，在面板那一层算成 px），全部相对面板字号（= body 的基准字号，**不跟** `ui.font_scale`；面板字号是常数，「字号同步调整表情尺寸」只对**弹幕行内**的表情成立） |
| 点选 = 直接发送 | **点一下就把这个表情发出去**：`chat_send` 立刻带上 `emote.emoticon_unique`（上游最终收到的 `msg` 就是它，见 `protocol.md` §11.4），**不往草稿里插名字、不需要再点「发送」**。必须记住「点的是哪一个」——表情名会重名（实测「贴贴」同时在通用包与房间包里）。草稿与 @ 目标原样留着（它们属于文字那一侧），已选「回复」时这一条表情就发成回复（发成功即清掉回复条）。面板**不关**：连发几个不必反复开面板。冒烟按 `ownedEmoteSentOnClick` / `ownedEmoteNoSecondStep` / `panelLockedEmoteSelectable` 断言 |
...
| `blocked_platform` | 被平台吞（命中**全局屏蔽词**） | 「发送失败 · 全局屏蔽词」[ · detail] | 浮动提示（§6.5.1）+ 行内标记（§4.4） | **保留** | 那条**留在列表里**、标成**被拒**：正文划线 + 行尾写该 outcome 的文案 |
...
| 最下方 | 最下方**不渲染任何提示元素**：输入区以下既没有发送结果那一行，也没有未登录的静态说明。冒烟 `sendFailNoBottomHint` / `sendFailToastGone` 断言底部无提示 |
...
2. **除 `ok` 外一律保留草稿**（被吞 / 被禁言 / 要粉丝牌 / 频率限制 / 上游报错都在内），可直接重试或
   照着行上划掉的那条改写。
3. 自动消失的时长就是 `SEND_TOAST_MS`（2600ms）；**没有手动关闭**，也没有
...
6. **可视高度变化要重新贴底**：弹出面板展开/收起、窗口缩放、礼物栏开合、**进退沉浸模式（§2.3.1）**都会改聊天区高度（`ResizeObserver` 监听滚动容器）。跟随模式下重新定位到末尾，否则最新弹幕会被挤出视口（§2.3「不遮挡最新弹幕」）；暂停状态下不重新贴底，而是**把锚点行按回它原来的屏幕位置**（进出沉浸模式都不把当前阅读位置弹走；容器自身的顶边也会挪，那一份按 §7.3 第 5 行的口径补回来）。
...
- 系统类消息（开播 / 下播 / 标题变更 / 公告）没有单独的门：第 1 步的白名单里勾着 `system` 才渲染。默认值不含它（需求 §2.1「默认关闭」，勾上「系统」才显示）。
...
| `interact` | 行色 `--fg-dim`（§4.1）；**列表里不渲染**（`ui.interact_single_slot` 开启时改由底部浮层呈现，§4.8） | 受 §4.8 的单槽位开关控制 |
...
主题由 `ui.theme` 决定（契约 §8：`system` / `dark` / `light`）；六种 `kind` 的颜色语义槽在明暗两套取值下分别定义，切换主题即换值，不改变 `kind` 语义。**主题控件在房间列表页页头「弹幕框」标题右侧**（需求 §2.8），不在房间页筛选面板里。形态是**一个按钮**，不是滑块：
...
**整体设计语言 = WhatsApp**：只取设计语言——色板 / 圆角 / 间距 / 字体层级 / 顶栏与输入栏形态，深浅两套跟随系统。以下是**明确不照搬**的部分：
...
| 按人给用户名上色 | 与既定口径冲突：用户名与正文都不吃上游自定义颜色（白字人名在浅色下会隐形）→ 昵称固定 `--fg-dim`、正文固定 `--fg`，身份靠徽标（§4.2） |

### 8.4 两条折叠规则：礼物连击折叠与弹幕聚合

行数变少的规则有**两条**，判据与对象都不同，**两条规则互不相干**：
...
| 其余 `kind` | 这条规则**不折**它们，一条消息一行。**跨观众**的弹幕聚合是**另一条**规则，见下 |
| 行数变化 | 折叠使行数减少，行高按 §7.1 重新测量、锚定按 §7.3 恢复 |
| 计数上限 | `N` 显示上限 999，超过仍显示 999（计数本身不封顶） |

**第二条：弹幕聚合（跨观众同文本）。** **不同的观众**在短时间窗口里发**同一条**弹幕够**多**时折成一行：**头像列**堆叠前几位的头像、**身份位**印「刷屏 ×N」，**一个用户名都不出现**。**三条公开常量**（窗口 / 折叠门槛 / 头像张数）加**一条判据常量**（观众门槛）与归一化规则都是规范性常量，见契约 §4 与 `aggregate.ts`。
...
| 窗口 | `AGGREGATE_WINDOW_MS = 5000`（`aggregate.ts:23`）= **5 秒**：与**上一条**（这一串的最后一条）比，**滑动** —— 每并入一条即把窗口往后刷一次 5 秒，**无条数上限**（只要相邻两条都落在 5 秒内，这一串就一直并下去，不因「离第一条超过 5 秒」而断开）。代表行仍是这一串的第一条（见下行），因此后续加入不重建节点、行不跳位 |
...
| 冒烟按 | `aggregateBlockRan` 与 `aggregate*` 一组（`25-aggregate-jump.mjs` **五条读数**）：① 同文本 + 3 位不同观众（窗口内）→ **一行**、身份位「刷屏 ×3」、堆叠 3 张头像（错位 30%、`z-index` 递减）、无用户名 / 无徽标 / 无行内 `×N`；② **不同文本** → 两行（只认同一个聚合键），且照旧画昵称；③ 同文本每 4 秒一条连发三条（首尾跨 8 秒、已超过一个窗口）→ 仍**一行**「刷屏 ×3」（**滑动**：基准是上一条，每并入一条刷新一次），再加一条离上一条 6 秒的 → 它自己单独一行；④ 三位观众里有一位**没头像** → 头像列只画两张、只错开一次；⑤ **关掉开关** → 同样三条**逐条显示**（三行、无「刷屏 ×N」、无堆叠层），点回来当场又折成一行 |

### 8.5 筛选与显示面板（工具行「筛选」）

房间页没有常驻的一整行控制条。所有筛选与显示开关都在输入区上方的**筛选面板**里，分两块；两块放得下就并排（宽屏）、放不下就上下排（竖屏 360，§9.1）：

| 分区 | 内容 | 相关键 |
|---|---|---|
| 消息类型 | 六种 `kind` 白名单（`filter.kinds`，全选=不过滤） | `filter.kinds` |
| 辅助功能 | 字号滑杆（占满一整行）、**时间戳**、**弹幕包含礼物**、**独立礼物栏**、**折叠低价礼物**、**剔除低价礼物统计**、**刷屏弹幕聚合**（默认开） | `ui.font_scale` / `ui.show_timestamp` / `ui.gift_in_danmaku` / `ui.gift_panel` / `ui.gift_collapse_cheap` / `ui.gift_exclude_cheap_stats` / `ui.danmaku_aggregate` |

**两块的表单是同一形态：两列勾选清单**：

| 项 | 规则 |
|---|---|
| 列数与铺法 | `grid-template-columns: repeat(2, minmax(0, 1fr))`，两块各自独立成网格。**按行铺**：左列 = 第 1/3/5/7 项、右列 = 第 2/4/6 项（DOM 序即阅读序 = `0101010`，键盘 Tab 与目视顺序一致）—— 消息类型因此是「弹幕 / SC / 大航海」一列、「礼物 / 互动 / 系统」一列；辅助功能**六枚**（「互动消息自动消失」那枚已随 `ui.interact_auto_hide` 删除）两列各 **3 / 3 项**、三行铺满（`27-filter-panel.mjs` 量到的几何） |
| 每一行 | 原生 `<input type="checkbox">` + 可见文字，**没有**芯片那层底色 / 描边 / 胶囊（不作按钮样）；`justify-items: start`，可点区域就是「复选框 + 文字」那一段，不留半行宽的隐形热区 |
| 两档宽度 | 同一份 DOM：宽屏两块并排、每块两列；窄屏（≤ 520px，含最小宽度 360）两块上下排，每块仍是两列 —— 360 下每列 ≈ 160px，最长的一条「剔除低价礼物统计」（8 个汉字 ≈ 96px + 复选框）放得下，**不许横向滚动** |
...
| 冒烟按 | `filterPanelTwoBlocks`（两块 = 消息类型 / 辅助功能）、`filterPanelKindLabels` / `filterPanelAuxLabels`（文案）、`filterPanelKindsTwoColumns` / `filterPanelAuxTwoColumns`（列 / 行几何：2 列、按行铺 —— 消息类型两列各 3 项、`010101`；辅助功能 **3 / 3 项、三行、`0101010`**，逐列项数由 `27-filter-panel.mjs` 量到）、`filterPanelTwoColumnLists`、`filterPanelPlainCheckboxList` / `filterPanelSameFormBothLists`（朴素复选框、两块同形态）、`filterPanelNoButtons`、`filterPanelNoHorizontalOverflow`、`filterPanelAuxComplete`（滑杆 + 六枚复选框、无 select）、`filterPanelRangeSpansRow`（滑杆那一行仍横跨两列）、`filterPanelAuxToggles`（六枚逐枚点开再点回，偏好与复选框同步翻）、`filterPanelTimestampUncheckedByDefault`（干净环境下复选框画的**就是**契约 §8 的默认值；`ui.interact_auto_hide` 已删除，「互动消息是否显示」现由 `ui.interact_single_slot` 这一枚复选框决定，见 §4.8）、`filterPanelTitlesProminent` / `filterPanelTitleCopyUnchanged`（两块标题的层级：字号 / 字重 / 字色 / 分隔线比清单项醒目，文案仍是「消息类型」「辅助功能」）断言 |
...
> 悬停提示（`title`）在这些开关与其他同类控件上只写**字段名**（控件名，如「时间戳」「剔除低价礼物统计」），不写操作教学、不写上游字段名与协议章节号。**例外是本身没有可见文字的控件**：主题按钮的 `title` / `aria-label` 必须带上当前档与下一档（§8.3）。

账号相关（登录态 / 多账号切换 / 新增 / 删除 / 扫码 / 登出）在**房间列表页的账号管理对话框**（§2.2.1），不占房间页空间。
其余键的读写仍走 `prefs_get` / `prefs_set`（写接受部分键值补丁，未知键或非法值返回 `BAD_REQUEST`）。
界面不提供粘贴 Cookie 的入口：登录只有扫码一条路，凭据文件由程序写回、界面没有改动它的路径（契约 §4.1、`auth.md` §8.4），因此凭据值不会经过前端。

---

## 9. 宽窄屏形态（断点与设计令牌）

界面要能**缩到竖屏（手机比例）**；全局风格成体系，样式不散在各处硬写。
§9 是排布与令牌的唯一口径，样式实现全在 `apps/desktop/ui/src/app.module.css`。

### 9.1 断点与形态对照

同一套 DOM，只有排布随宽度变。断点是 **520px**：> 520px 是宽屏（桌面），≤ 520px 是窄屏（竖屏）。
**竖屏是默认形态**（窗口默认尺寸 390×844，见 `apps/desktop/src-tauri/tauri.conf.json`），宽屏是适配；
排版的**第一优先**因此是窄屏，宽屏是同一套排布在更宽容器里的自然结果。
窄屏的验证视口取**窗口最小宽度 360×844**（可达面的边界值，`testing.md` §9），不是 390：390 只是常见值，
...
把窗口拖到 **360–520px** 之间就是窄屏形态，桌面端可自查。
安卓端将来复用同一套断点与令牌，不另开一套样式。
...
因此宽屏那两条口径（`layoutOnlyChatShrank` 只挤列表、`layoutNewestNotCovered` 不遮最新一条）在**两个视口都是断言**，没有例外名单（`testing.md` §9）。另外，面板展开会改可视高度，用户自己滚上去看历史时**正在看的位置不能被弹走**（`layoutPanelScrollStable`：停在中间 → 展开 → 同一行在视口里的位移 < 8px）；跟随模式下重新贴底是有意的（`MessageList` 的 `ResizeObserver`）。
...
| 行与触屏的固定尺寸 | `--row-line`（行盒高）、`--avatar` = 1.25×行盒、`--badge-h` = 0.9×行盒（**不跟头像走**）、`--emote` = 1.1×行盒、`--time-col`（= 8ch：时间戳那一格的定宽，位置在**身份行右端**，见 §4.1）、`--tap-min`、`--ctl-round`（= `--tap-min`：圆形控件的直径 —— 房间头两枚「返回与 `⋯`」、回到最新、主题按钮，以及房管面板的 X 关闭与批量图标钮，同一个控件族）、`--live-dot`（= 8px）、`--tab-min-w`（= 96px）、`--title-gap`（= 2.5em）、`--splitter-hit`（= 8px，分割条热区）、`--panel-max-h`、`--panel-max-h-narrow`、`--sheet-max-h` | — | 只有这些是「结构尺寸」；其余一次性尺寸（二维码 200px、菜单最小宽 150px 等）仍写在各自规则里，不硬凑进间距阶。`--panel-max-h-narrow` 用视口份额（45vh）而不是固定像素，窄屏才不会被面板吃掉整屏。**弹幕行的那四个是一组**：`--row-line` 是基准（行盒高，1.5em，声明在 `.row` 上并用 `@property` 注册成 `<length>`，这样它算成 px 后再往下继承，子元素各写各的字号也不会算错）；**头像（1.25×）与身份牌（0.9×）是两条独立的派生**（复用时牌会跟着头像一起放大）；**而 `--avatar` 有两层**：`.row` 里是行盒的 1.25 倍，`:root` 里另给一个行外默认值（1.35em，随所在字号走）——`.avatar` 是跨页面复用的组件（关注列表 / 账号区 / 账号对话框都在弹幕行之外），令牌只活在 `.row` 里会让那些地方 `var(--avatar)` 解析失败、`<img>` 退回原图尺寸（回归断言见 `testing.md` §9），头像 / 身份牌 / 表情三个尺寸都从基准按比例派生（§4.1） |

**§3 / §9 引用到的令牌逐项对照**（「定义」列给 `app.module.css` 的行号；「值」列是深色基座 `:root` 那一套，浅色换值见下面的双套取值表；`--gesture-*` 由原生外壳运行时下发、不在 CSS 里定义）：

| 令牌 | 定义 | 值 | 正文引用处 |
|---|---|---|---|
| `--accent` | `app.module.css:36` | #00a884 | §9.2 |
| `--accent-text` | `app.module.css:39` | #00a884 | §9.2 |
| `--avatar` | 两层：`.row` 内 `app.module.css:986`（= 1.25 × 行盒）、`:root` 默认 `app.module.css:150`（1.35em，行外共用组件用） | calc(var(--row-line) * 1.25) / 1.35em | §9.2 |
| `--badge-border` | `app.module.css:90` | color-mix(in srgb, var(--badge-lift) 45%, transparent) | §9.2 |
| `--badge-fg` | `app.module.css:88` | #fff | §9.2 |
| `--badge-h` | `app.module.css:987` | calc(var(--row-line) * 0.9) | §9.2 |
| `--badge-lift` | `app.module.css:89` | #fff | §9.2 |
| `--bg` | `app.module.css:30` | #0b141a | §9.2 |
| `--bg-elevated` | `app.module.css:31` | #111b21 | §9.2 |
| `--bg-input` | `app.module.css:32` | #202c33 | §9.2 |
| `--block-platform` | `app.module.css:81` | #d93025 | §9.2 |
| `--block-room` | `app.module.css:82` | #f0a020 | §9.2 |
| `--border` | `app.module.css:33` | #2a3942 | §9.2 |
...
| `--fg-subtle` | `app.module.css:57` | #6a7e8a | §9.2 |
| `--fold-line` | `app.module.css:101` | var(--fg-subtle) | §9.2 |
| `--fs-1` | `app.module.css:119` | 0.79em | §9.2 |
| `--fs-5` | `app.module.css:123` | 1.07em | §9.1 |
| `--fs-8` | `app.module.css:126` | 1.43em | §9.2 |
| `--fs-root` | `app.module.css:128` | 14px | §9.2 |
| `--gesture-left` | 原生外壳运行时下发（`back.ts:79-85` 读取） | 设备 inset 换算值 | §9.3 |
| `--gesture-right` | 原生外壳运行时下发（`back.ts:79-85` 读取） | 设备 inset 换算值 | §9.3 |
| `--gold` | `app.module.css:71` | #c08a2e | §9.2 |
| `--guard-1` | `app.module.css:73` | #c0392b | §9.2 |
| `--header-bg` | `app.module.css:176` | color-mix(in srgb, var(--bg-elevated) 88%, transparent) | §3.1、§9.2 |
| `--header-lift` | `app.module.css:177` | color-mix(in srgb, var(--fg) 5%, transparent) | §9.2 |
| `--hover-wash` | `app.module.css:92` | color-mix(in srgb, var(--accent) 8%, transparent) | §9.2 |
| `--live-dot` | `app.module.css:162` | 8px | §3.1、§9.1 |
| `--live-idle` | `app.module.css:54` | var(--fg-dim) | §3.1、§9.2 |
| `--live-off` | `app.module.css:53` | var(--danger) | §3.1、§9.2 |
| `--live-on` | `app.module.css:52` | var(--ok) | §3.1、§9.2 |
| `--mention` | `app.module.css:67` | #fb7299 | §9.2 |
| `--neutral` | `app.module.css:72` | #6b7280 | §9.2 |
| `--ok` | `app.module.css:42` | #25d366 | §9.2 |
| `--on-accent` | `app.module.css:85` | #0b141a | §9.2 |
...
| `--r-full` | `app.module.css:116` | 999px | §9.2 |
| `--row-line` | `app.module.css:985` | 1.5em | §9.2 |
| `--safe-bottom` | `app.module.css:204` | env(safe-area-inset-bottom, 0px) | §9.3 |
| `--safe-top` | `app.module.css:203` | env(safe-area-inset-top, 0px) | §9.3 |
| `--sc-1` | `app.module.css:76` | #2f6fd0 | §9.2 |
| `--select-wash` | `app.module.css:96` | color-mix(in srgb, var(--accent) 22%, transparent) | §9.2 |
| `--shadow-dialog` | `app.module.css:157` | 0 8px 28px rgba(0, 0, 0, 0.48) | §9.2 |
| `--shadow-menu` | `app.module.css:156` | 0 4px 16px rgba(0, 0, 0, 0.4) | §9.2 |
| `--shadow-sheet` | `app.module.css:158` | 0 -6px 20px rgba(0, 0, 0, 0.45) | §9.2 |
| `--sheet-max-h` | `app.module.css:183` | min(80vh, 640px) | §9.2 |
| `--sp-0` | **未定义**（§9.2 里只是「新增一档紧凑间距」的举例，代码中不存在） | — | §9.2 |
| `--sp-1` | `app.module.css:105` | 4px | §9.2 |
| `--sp-2` | `app.module.css:106` | 8px | §9.1、§9.2 |
| `--sp-3` | `app.module.css:107` | 12px | §3.1、§9.1 |
| `--sp-5` | `app.module.css:109` | 24px | §9.2 |
| `--splitter-hit` | `app.module.css:190` | 8px | §5.4、§9.1 |
| `--tab-min-w` | `app.module.css:167` | 96px | §2.3、§9.1 |
| `--tap-min` | `app.module.css:161` | 40px | §9.1、§9.2 |
| `--time-col` | `app.module.css:153` | 8ch | §9.2 |
| `--title-gap` | `app.module.css:168` | 2.5em | §9.1 |
| `--toast-bg` | `app.module.css:174` | color-mix(in srgb, var(--danger) 22%, var(--bg-elevated)) | §9.2 |
| `--toast-fg` | `app.module.css:175` | var(--fg) | §9.2 |
| `--warn` | `app.module.css:41` | #e9a038 | §9.2 |
...
| `--live-idle` | `#8696a0`（= `--fg-dim`） | `#54656f`（= `--fg-dim`） | 现状（不改色值） | 状态点「**未连接**」**灰**（连接态不是 `connected` **或**列表载荷的 `connected` 为假）。灰取自次级文字那枚中性灰（饱和度 12% / 14%），非文字图形要素对 `--bg` / `--bg-elevated`：6.1:1 / 5.7:1（深）、5.1:1 / 6.1:1（浅），≥ 3:1 ✓ |
...
| `--fold-line` | `var(--fg-subtle)` = `#6a7e8a` | `var(--fg-subtle)` = `#667781` | 派生（引用已有槽） | 上下分区那条**分界线**（§5.4）：对画布 **4.4:1 / 3.88:1** ✓（非文字图形要素 ≥ 3:1） |
...
**关于「挤了怎么办」的一条规矩**：弹幕行的行高是 **内容 + `--sp-2`（8px）上下内边距 ≈ 58px**（两行：身份行 21px + 正文行 21px + 16px），触屏上行与行要分得开；代价是同样高度少看几行——面板展开时弹幕列表约剩一半（360×844 下约 390px ≈ 6 行 @58px），面板内容超出则走面板**内部滚动**，不靠遮挡换空间。如果将来真觉得挤，**正确的做法是新增一档「紧凑」令牌**（例如 `--sp-0: 2px`，并在令牌表里写清它只用于哪一类场景），**不是**就地把某个 `--sp-2` 改成 `4px`：间距阶的价值就在于「以后不用微调」，破一次例就等于给下一个人留了口子。

### 9.3 视口、系统栏与软键盘（谁负责避让；需求 §2.11）
...
实测（模拟器 android-35，1080×2400@420dpi，WebView Chrome/124，房间页弹幕列表有内容）：
...
**为什么选「自补内边距」而不是「让系统 resize」**：键盘高度只有原生拿得到，而它本来就已经把 `ime()` 算进 `--safe-bottom`（不然输入区会压在键盘下面）；窗口再 resize 一次就是双重避让 —— 视口少一截、内边距又少一截，内容比视口高，**`document.scrollingElement` 就变成一个可滚容器**，**滚动接力**会把手势从弹幕列表带到文档，整个界面（含房间顶栏）被顶上去，底边露出一条画布色。所以 `adjustNothing` 不是可选优化，是把「只有一条机制」钉死的那颗钉子：默认的 `adjustUnspecified` 在别的系统版本上可能解析成 `adjustResize`（同样造成双重避让）或 `adjustPan`（整窗平移，顶栏直接滑出屏幕）。

**内核那一半：WebView 版本会自己动视口（M139 起）。** 键盘避让的另一半不归系统管，归内核：按官方文档 [`Understand window insets in WebView`](https://developer.android.com/develop/ui/views/layout/webapps/understand-window-insets)，**WebView M139 起 `ime()` 会直接缩「视觉视口」（只缩底部）**，而 M139 之前内核**什么都不做**（只把 inset 交给页面）。这条差异在模拟器上是可测的：WebView 113 / 133（android-34/35/36 镜像）上，键盘弹出前后 `window.visualViewport.height` 恒为 915 CSS px、`offsetTop` 恒为 0（内核没动过视口，全靠 `--safe-bottom` 那条内边距）；M139 起视觉视口会缩到「键盘上沿」，官方口径是**页面因此在键盘下方变得可滚**、手指一滑就能把内容顶上去 —— 也就是「整个界面被顶走、底边露出画布」的那条路。

**「文档不可滚」是硬约束**。实现就三行：`html` / `body` 高度用 `100dvh`（不支持的引擎落回 `100%`），三层 `overflow: hidden`，根上 `overscroll-behavior: none`。冒烟按三条断言钉住它（都是**结构**断言，不依赖 CSS-module 类名）：`docNeverScrollable`（三种状态下 `scrollHeight - clientHeight <= 1` 且 `scrollTop === 0`）、`docHeightsMatchViewport`（`body` 铺满视口，`#root` = 视口 − `body` 的上下内边距 —— `#root` 的 `height: 100%` 解析的是 `body` 的**内容盒**，而 `body` 的 `padding-bottom` 就是让开键盘的那条 `--safe-bottom`，所以键盘那一档 `#root` 正好短掉 336px 是**让开本身**，不是漏让开）、`docComposerVisibleWithKeyboardInset`（把 `--safe-bottom` 换成 336px 这一档时输入框仍在视口内）。三种状态取「常态 / 面板展开（固定高度的兄弟最多的一档）/ 键盘内边距」。
**无头冒烟里没有 IME**，它覆盖的是这条链子的**布局那一半**（内边距换掉之后文档仍不可滚）；**IME 那一半（系统 / 内核会不会额外 resize 视口）只能在设备上实测**。
...
## 11. 空态与错误态（需求 §2.1 / §2.4 / §2.5 / §2.7 / §2.8 / §2.12）
...
> 唯一「待实测校准」表在 [`protocol.md`](protocol.md) 附录 A。

---

## 14. 一致性约束

1. UI 侧出现的 IPC 命令、事件名以 [`ipc.md`](ipc.md) 与契约 §7 为准；六种 `kind`、`Message` / `RoomSession` / `Emote` / `FollowedRoom` / `SendOutcome` 字段以契约 §5 为准；偏好键名、类型与默认值以契约 §8 为唯一权威清单——UI 不新增、不重命名，也不把非 §8 状态写入 `prefs.json`。
2. 时间统一按 UTC 毫秒（`ts`）传输，界面按本地时区渲染（`HH:mm:ss`）；跨天时在时间前补日期。是否**显示**时间戳由 `ui.show_timestamp` 决定（默认关闭，见 §8.5），渲染格式与对齐规则不变。
3. 新增 `kind`、调整过滤求值顺序或调整偏好键用法时，必须同步更新 `ui.md` 与 `CHANGELOG.md`；若需新增偏好键，先改契约 §8 再改 `ui.md` 与 `ipc.md`。

---

## 15. 自检：房间页无头冒烟

房间页无头冒烟的跑法、验证矩阵（**2 引擎 × 2 视口 × 2 主题**）、运行纪律、断言契约表与失败判读属**验证面**，唯一口径在 [`testing.md`](testing.md) §9；此处不再重复，也不在别处自建断言清单。
