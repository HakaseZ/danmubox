...
│ │本房间（10）│ ← 分组 tab 轨道（竖向、在自己的列里滚）         │
│ └粉丝牌（17）┘  网格区固定两行大表情，超出滚动                 │
├──────────────────────────────────────────────────────────────┤
...
| 身份簇的宽度上限 | 身份簇再长（长昵称 + 三个徽标 + 回复标记）也不许把正文挤成一条缝：`.identity { max-width: var(--identity-max-w) }`，宽屏 **14em**（与 `.name` 同档，不会更早截断）、窄屏（≤520px）**10em**（14px 字号下 140px，约正文块的一半）。用户 2026-09-12：「文字全挤在右边，没有办法往左边用户名、身份牌等下方自动换行」——改前实测（WebKit，360×844，那条真实弹幕）：身份簇列 **182.7px**（占正文块 56%）、正文列只剩 **112.4px**；改后第 1 列 **140.0px**、正文列 **155.1px**、正文起点 x 从 **225.6 → 182.9**，仍折行（4 个行盒、悬挂缩进对齐）且横向溢出 0。**宽屏一字未动**（1440 下身份簇 182.7px < 14em，正文起点仍是 225.6px）。⚠ 上限**必须用长度**：`minmax(0, 50%)` 会被网格的 maximize-tracks 撑满 50%，每行的正文起点被推到行中间（实测 225.6 → **734.4px**）。昵称是簇内唯一允许收缩的一格（`.name` 的 `flex: 0 1 auto` + `min-width: 0` + 省略号），徽标组保持自己的宽度不被压扁。冒烟按 `fixtureTextColumnKeepsHalf` / `fixtureTextNoOverflow` / `fixtureTextWrapsWhenNarrow` 断言（夹具见 §15） |
...
**网格区高度 = 两行大表情**（用户 2026-09-12：「既然表情包做了滚动，展开只展示 2 行（大表情的 2 行，以此高度为标准）就行」）：`.picker` 上算好 `--emote-size-big = 2.2 × --fs-7`（注册成 `@property` 的 `<length>`，在面板那一层就算成 px 继承下去）、`--emote-row-h-big = --emote-size-big + 2 × --sp-1`（图片盒 + 格子上下内边距）、`--emote-grid-h = 2 × --emote-row-h-big + --sp-1`（两行 + 一道行距），网格取 `height: var(--emote-grid-h)`、超出滚动。**高度写死而不是 `max-height`**：换分组时面板高度不跳，滚动条该出就出。注册 `<length>` 是硬要求——不注册的话同一个 `calc()` 到了格子那一层会按格子自己的字号（`--fs-7`）再乘一次，两行的高度会差 1.3 倍（冒烟 `panelEmoteGridTwoBigRows` 按「网格高 == 2 × 大格子高 + 行距」断言，`panelEmoteGridScrolls` 断言超出会滚）。
...
| 滚动 | 轨道自己滚（组多时）、网格自己滚，**面板头与轨道都不动**——tab 是切组的唯一入口，被格子滚走就得先滚回顶部（用户 #6） |
...
| 网格 | 等宽格子 `repeat(auto-fill, minmax(4.5em, 1fr))`（列宽下限按通用表情 200×60 的长宽比定：3em 时 `contain` 后图只剩 7px 高，认不出是哪个），一行放几个由列宽定；格子里的图居中。**表情格是面板里唯一会滚的部分**（面板头与轨道常驻）；网格区高度 = **两行大表情**（见上），因此面板整体高度也只在「面板头 + 两行大表情 + 轨道」这一量级上。窄屏 360 下面板 / 轨道 / 网格三块都**不许出现横向溢出**（冒烟 `panelNoHorizontalOverflow`）。长列表虚拟化或分页加载 |
...
| 网格区高度 vs 两行大表情（360×844） | —（网格被面板限高 `45vh` ≈ 380px 撑满，面板高 380px） | **相等**：网格 **100.1px** = 两行 **48.0px** + 行距 **4.0px**（`panelEmoteGridTwoBigRows`），面板高 **257px**；通用组内容 568px → 超出滚动（`narrow_panelEmoteGridOverflows`） |
...
| 弹幕列表 | 唯一生长区与滚动区 | 同左 |
| 弹出面板（表情 / 短语 / 筛选） | 文档流里的一块，向上展开，只挤压列表（不遮最新一条）；限高 `--panel-max-h` = 260px。**表情面板是例外**：网格区高度自己定死为**两行大表情**（`--emote-grid-h`，§6.3），因此面板实际高度远小于 260px（面板头 + 两行 + 轨道），滚动只发生在网格里 | **同一口径**：也在文档流里，只挤压列表、不遮最新一条；区别只在限高——窄屏用视口份额 `--panel-max-h-narrow` = **45vh**（844px 下 ≈ 380px），内容超出由**面板内部滚动**承担，不去吃列表空间。表情面板同样按两行大表情定高（比 45vh 更矮）。面板头常驻「关闭」按钮 |
