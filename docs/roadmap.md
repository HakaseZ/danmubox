...
| 项 | 说明 | 触发 / 前置 |
|---|---|---|
| 连线礼物映射 | `UNIVERSAL_EVENT_GIFT(_V2)` → 礼物条目。**A22 结案后只剩这一项**：`PK_INFO` / `RANK_CHANGED_V2` / `WIDGET_BANNER` 已按需求展示面判定为有意忽略（`protocol.md` A22） | 需 PK / 连麦活动期房间的载荷样本（一次 40 秒观察里一条未现）。社区文档有字段表，但嵌套结构未实测，按项目规矩不得凭文档猜结构 |
