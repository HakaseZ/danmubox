...
| A17 | `SendOutcome` 各错误码 | `rate_limited` / `medal_required` / `muted` / `failed` 各自对应的上游 `code` 与 message 文案 | 同上；需构造频率限制、粉丝牌不足、被禁言三类场景各一次 | 记录 `code` + message + 场景，建立归一化映射表。**已观测**：`code=10023`「发送失败，请先移除该用户黑名单」（登录态、公开测试房间 `1` 发送时触发），当前归为 `failed` 并保留原始 code | §11.3、UI 失败提示 |
