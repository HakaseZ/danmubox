...
| P26 | 整体设计风格模仿 WhatsApp：**全应用一次换完**，深浅两套配色并**跟随系统**；只取设计语言（色板/圆角/间距/字体层级/顶栏与输入栏形态），**不照搬气泡结构**（弹幕是多人流水） | 用户 2026-09-12 对话（选「全应用一次换」+「深浅两套跟随系统」；无截图，按设计规则做） | 未做（进行中：规范票产出映射后实现） | 待实现；映射规范见 WhatsAppSpec 交付；现有主题机制 `ui.theme`（契约 §8）+ `app.module.css` 令牌层为落点 |
