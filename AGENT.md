...
### 6.5.5 阅读纪律（默认不读 CHANGELOG）

**默认不读 `CHANGELOG.md`**，除非任务**涉及过去的改动** —— 查某条规范的由来、某次修正的理由、某个行为是什么时候变成现在这样的。
理由：它是全仓体量最大的文档（含末尾归档区），通读会挤占本该用于代码与规范文档的上下文。

- **写条目**时只读 `[Unreleased]` 段（§6 要求「随改动同一次提交写入」），不必通读历史。
- 真需要查历史时**按节定位**（浅层检索、跳到对应版本段或归档分节），不要整篇读；末尾归档区尤其如此。

### 6.5.6 需求入口（2026-09-19 起）
