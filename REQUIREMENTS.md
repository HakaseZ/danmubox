...
**2026-09-24 三次修订（issue202609242158）**：又在二次修订之上按用户该 issue 的八条要求改口径与体验（**本条为最终口径**，落点 `ui.md` §2.2.1 / §2.2.2 / §4.9 / §6.3 / §6.4、`ipc.md` §3 / §8.1、`contract.md` §7、`protocol.md` §18.1 / 附录 A66-1、`testing.md` C-16）：

- **入口同排、右对齐**（第 1 条，**推翻**上一条的「独立成组 + 竖分隔」）：「我的直播间」按钮与账号管理按钮（重新登录 / 退出登录 / 删除）**同一排、同一组**，整组贴账号行右端，**不再有竖线分隔**（用户原话「保持和用户状态管理的几个按钮在同一排，也不需要竖线分隔，又对齐就行」）— 落点 `ui.md` §2.2.1 / §2.2.2。
- **切换分区的修复口径**（第 2 条）：改分区走的仍是 `area_id`（**字段名不动**，`area_v2` 只属开播）；真因是 `Area/getList` 的**子分区 `id` 可能是数字字符串**，而解析只认 JSON 数字 → 子分区被丢掉、界面回落到父分区 id → 上游按子分区表校验回 `60009 分区已下线`；现两段式收（`as_u64` → 字符串 `parse`）— 落点 `protocol.md` §18.1 / 附录 A66-1。
- **标题与开播即时性**（第 3.1 / 3.2 条）：标题保存后预览**不再跳回** —— `anchor_title_set` 返回值以**本次请求值**为准（后端把请求标题并进重读结果），前端落返回值 + 读写发起序号护栏 + 15 秒写穿窗口；开播 / 下播**乐观落 `live_status`**，再由展开期静默轮询校正 — 落点 `contract.md` §7、`ipc.md` §3 / §8.1、`ui.md` §2.2.2。
- **红绿状态**（第 4 条）：未开播 / 直播中用红绿语义色更明显地区分（复用房间头同一套 `--live-on` / `--live-off`）— 落点 `ui.md` §2.2.2。
- **选择框等高**（第 5 条）：标题输入框与分区选择框共用一个显式盒模型、固定高度，`select` 关 macOS 原生外观自绘箭头；**macOS 已改，Windows 待实机确认**（不臆造）— 落点 `ui.md` §2.2.2。
- **FaceAuth 去红字**（第 6 条）：人脸认证那条「客户端老了」是无效提示（已有浏览器认证入口），**不再显示红字**；`QrConfirm` 的上游原话照旧保留 — 落点 `ui.md` §2.2.2。
- **展开期静默刷新**（第 7 条）：「我的直播间」展开期 30 秒静默重拉 `anchor_room`，收起 / 关对话框 / 换号即停 — 落点 `ui.md` §2.2.2、`ipc.md` §3.1。
- **全系统静默刷新审计**（第 8 条）：新增房管面板展开期 60 秒、表情面板打开按 10 分钟节流、发送成功后刷电池余额（不另起定时器）；审计判定**不必修**的几处 —— 房内弹幕 / 连接态 / 房内身份 / 房间元信息由 WS 事件实时推送，关注列表随列表页同一拍刷新，扫码登录已有 2 秒轮询，分区列表是静态数据 — 落点 `ui.md` §4.9 / §6.3 / §6.4、`ipc.md` §3.1。
