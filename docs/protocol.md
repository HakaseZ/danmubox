...
| A30 | 历史弹幕接口 | 是否可在进房间时取到最近若干条弹幕；上限；字段与实时弹幕的差异 | 官方页面前端产物中检索 `dM/` 路径，并在在播房间上实测计数 | **已实测（2026-09-12）**：`GET https://api.live.bilibili.com/xlive/web-room/v1/dM/gethistory?roomid=<真实房间号>`；返回 `data.room`（普通用户**恰好 10 条**）与 `data.admin`（房管**最多 10 条**，实测某房间两者皆满 = 20 条）。**上限 10+10，不可翻页**：`limit` / `page_size` / `size` / `ps` / `page` / `offset` / `last_id` 实测均不增加条数。**无需登录态**（带与不带 Cookie 实测同结果）。条目字段与实时 `DANMU_MSG` 不同但足够渲染：`text` / `uid` / `nickname` / `timeline`（`yyyy-MM-dd HH:mm:ss`，秒级）/ `medal` / `guard_level` / `isadmin` / `user_level` / `id_str` / `user.base.{name,face}`。**坑**：探测过程中出现过数次 `code=0` 但两个数组皆空（同一请求稍后即返回 10 条，机制未知）——调用方**不得把空数组当成错误**，也不得据此判定房间无弹幕 |
