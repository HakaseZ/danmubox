...
- **列表自己会刷状态**（周期与三条闸门见 `contract.md` §4）：
  ...
  **③失败退避**（30 秒只属正常周期；失败后按 **60 → 120 → 240 秒**逐次加倍并封顶，成功即复位 —— `store.ts:559-531`、`:578-582`）。
...
| `AccountManager.tsx` | 账号对话框：账号列表（非当前行整行可点即切换）、状态、重登 / 登出 / 删除、确认条、扫码（§2.2.1）；**外加「我的直播间」区域**（§2.2.2）：直播间标题（可改）、开播状态与开播 / 下播按钮、双击状态文本展开「相关配置项」（当前分区 / 推流地址 / 推流码，默认隐藏）——渲染条件、两行布局、隐藏口径与 `db-anchor-*` 钩子都在 §2.2.2 |
...
| 折叠的形状（**两处各折一次**） | 低价礼物在**弹幕区**与**礼物栏**里**各**合并成一条：取桶里**第一条**的昵称 / 头像 / 礼物名与**位置**（新礼物到来只改这一行的 `×N` 与金额，行不跳位、React key 不变），`×N` 与金额是整桶合计。两处各持自己的行集合（同一个函数、各调一次，`filtering.splitGiftRows`），互不影响；金额格只有礼物栏那一份画（§5.3「金额与单位」），因此弹幕区的桶行只多出一个 `×N` |
