...
| `gift` | 竖条 `--gold #C08A2E`；正文是上游给的「`<动作词> <礼物名>`」；行尾 `×N` | `礼` | 左对齐 | 正文档 / 昵称低一档 | 行里**不画金额**（上游把金额放在 `amount` 而不是正文里，行内再塞一格会与 `×N` 挤在一起）；金额在**独立礼物栏**里按「`<数量> 瓜子`」呈现，单位见 §5.3 与 `protocol.md` 附录 A |
...
│                                                            (↓) │      内部滚动，唯一生长区是这一块分区
├──────────────────────────────────────────────────────────────┤   ← 分割条（常驻，热区 8px，可拖）
│ 礼物 / SC（6）  本场 礼物 3 · 700 瓜子 / SC 2 · 1,030 元 /      │   ← 礼物折叠条（折叠态只有这一行）
│                 大航海 1 · 138,000 瓜子               收起 ▴  │
├──────────────────────────────────────────────────────────────┤   ← 展开后：栏内独立滚动（栏高由份额定）
│ (头像) 王五  投喂 小心心      ×3   600 瓜子                    │
│ (头像) 周八  投喂 辣条        ×1   100 瓜子                    │
│ (头像) 赵六  这是醒目留言正文      30 元                       │
...
| 金额与单位 | 金额格 = `filtering.amountText(amount, kind)`：礼物与大航海是**金瓜子**、SC 是**元**（契约 §5 既定口径）。**不做任何换算**（SC 载荷里的 `rate` 未核验），也**不做跨单位求和**：汇总与每行都各带各自单位 |
| 折叠态汇总 | 标题行 =「礼物 / SC（N）」+ **按 kind 分组**的汇总（每组 `<kind> <条数> · <金额><单位>`，空组不出现），例如「本场 礼物 3 · 700 瓜子 / SC 2 · 1,030 元 / 大航海 1 · 138,000 瓜子」；金瓜子与元**绝不加到一个数里**。分组明细是一个可缩的省略项（`text-overflow: ellipsis`）：窄屏下放不下就截断，标题上那个总条数不缩 —— 明细是给宽屏「一眼看清」用的，条数才是窄屏的兜底 |
