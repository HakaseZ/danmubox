...
| F-08.1 | 上下分区的比例与顺序 | 拖动分割条、长按换位、改 `ui.gift_pane_ratio` / `ui.gift_pane_on_top` | 拖动实时改两栏高度（拖动中不写 store，松手写一次）、长按 0.5s 换位、两枚键持久化并在重启后读回；最小高度双向生效（弹幕区 ≥ 3 行、礼物栏 ≥ 总计条）；`ui.gift_panel` 关掉时退化为弹幕区全高、分割条消失（`ui.md` §5.4；无头冒烟同款断言见 `smoke/scenario/parts/34-split-panes.mjs` 的 `splitter*` / `swap*`） |
| F-08.2 | 低价礼物（≤ 0.1 元）的折叠与统计剔除 | 勾上「辅助功能」里的「折叠低价礼物」/「剔除低价礼物统计」，**弹幕区与礼物栏两处**都放 0.09 / 0.10 / 0.11 元与没给价（`amount = 0`）四条礼物 | **两个区域**（弹幕区 + 礼物栏）各量一次：折叠让**两处**的低价礼物合成一条（`×N` 与金额是整桶合计，弹幕区那一处不画金额格），总计条的统计逐字不变；剔除只改**总计条**的「礼物 / SC（N）」与三组明细（统计面只有这一处），**两处的行都不动**；边界：0.09 / 0.10 算低价、0.11 与 0 不算；SC 与大航海不受两枚开关影响；两枚默认都关且持久化（`ui.md` §5.3、`contract.md` §8；无头冒烟同款断言见 `smoke/scenario/parts/35-cheap-gift.mjs` 的 `cheapGift*` 与 `switchScope*`，单测见 `src/filtering.test.ts`；**桶行的 ×N 在身份位**（`db-msg-spam` / `db-gift-spam`，写「低价礼物 ×N」），正文那格 `db-msg-count` **不画**（与刷屏聚合同一套形态，见 `switchScopeBucketNoInlineCount`） |
| F-08.3 | **剔除 / 折叠 / 隐藏 / 自动消失都不丢内容、开关关掉即复原**（issue 2609171849 第 5 条） | ① 折叠开着时关掉它：**两个区域**各自逐条回来；② 剔除开着时关掉它：统计串逐字回来；③ 拨掉「互动」那一项（它绑 `ui.interact_single_slot`）：互动消息完全不显示（列表与浮层都不画、预留高度一并收回），拨回来按同样顺序出现；④ 关掉 `ui.gift_panel` / `ui.gift_in_danmaku`：被藏起来的那一批整批回来 | 这四种机制**都只是显示层的派生**——原始消息始终留在会话缓冲里（只受契约 §4.3 的上限约束），不得因它们提前消失。关掉后顺序、数量、金额与统计串逐项与开之前相同（`ui.md` §5.3「不丢内容 / 可逆」与 §4.8；单测 `src/filtering.test.ts` 第 2/3/5 条，冒烟 `switchScope*`） |
| F-08.4 | **刷屏弹幕聚合**（不同观众在同一窗口里发同一条弹幕折成一行） | 五档：同文本 + 3 位不同观众、不同文本、同文本但第一条落在窗口外、三位观众里有一位没头像、关掉 `ui.danmaku_aggregate` 再点回来 | ① 同文本 + 3 位观众：折成**一行**，身份位印「刷屏 ×3」（`db-msg-spam`）、头像列（`db-msg-avatar-col`）里画 `db-msg-avatar-stack` 的 3 张堆叠头像（沿 X 每张错开 30% 头像宽、后一张压在前一张上、**最左那张 z-index 最高**），身份位**不画**用户名（`db-msg-name`）与徽标（`db-msg-badges`）、行内**不画** `×N`（`db-msg-count` 只属于礼物连击与低价礼物桶）；② 不同文本：两行（只认同一个聚合键）；③ 同文本每 4 秒一条连发三条（首尾跨 8 秒）→ 仍**一行**「刷屏 ×3」（**滑动**：基准是上一条，每并入一条刷新一次），再加一条离上一条 6 秒的 → 它自己一行；④ 一位观众没头像：头像列只画两张、只错开一次（空 `face` 的那位不画，错位按实画张数算，头像列本身照留宽）；⑤ 关掉开关：同样三条**逐条**显示（无「刷屏 ×N」、无堆叠层），点回来当场又折成一行。单测见 `src/aggregate.test.ts`（10 条）；无头冒烟同款断言见 `smoke/scenario/parts/25-aggregate-jump.mjs` 的 `aggregateBlockRan` / `aggregate*` 一组（`ui.md` §8.4 第二条、`contract.md` §8） |
...
| C-14 | 共享分区：拖分割条、长按换位、关掉礼物栏 | 拖动两栏之间的分割条：高度**实时**跟着走，松手后比例留在 `prefs.json`（`ui.gift_pane_ratio`），重开应用仍是这个比例；拖到极限时弹幕区不短于 3 行、礼物栏不短于它的总计条（不压到 0、不溢出）；长按任一栏 0.5s 后拖到另一栏松手：两栏上下互换且 `ui.gift_pane_on_top` 落盘（拖回本栏或按 ESC 取消）；在筛选面板关掉「独立礼物栏」：礼物栏与分割条一起消失、弹幕区占满整块（`ui.md` §5.4）。鼠标与触摸各做一遍 |
...
| C-16 | 「我的直播间」（账号对话框里**按账号展开**，issue202609241553 改口径）：**没开通 / 未登录不显示按钮**、**打开对话框即预取各账号房间**、**不必切号即可跨账号管理**、**分区改动即存**、**ROOM_CHANGE 自动更新标题**、**下播后推流信息不入 DOM** | ① 账号对话框里**每个账号行最右端**有一枚「我的直播间」按钮（`db-anchor-toggle`）—— 它与账号管理按钮（重新登录 / 退出登录 / 删除）**同一排、同一组**（都挂在 `.accountActions`；issue202609242158 第 1 条**撤销**了 issue202609241553 的「独立成组 + 竖分隔」），整组贴行右端、**不再有竖线分隔**；**只有「已登录且 `anchor_room` 给出直播间」的行才有这枚按钮**——没开通（后端 `anchor_room` 返回 `null`）/ 未登录的行**不渲染**（`anchorToggleOnRowWithRoom` / `anchorNoToggleOnNoRoom` / `anchorNoToggleOnGuest`）；按钮在「删除」**之后、同一排**（`anchorToggleRightOfRemove`）；**点这枚按钮不冒泡到账号行**（`.accountActions` 统一 `stopPropagation`）——它是「看 / 管**这个账号**的直播间」，**不是切号**（切号会把刚展开的管理区与按账号映射一并清掉）；**打开对话框即预取所有已登录账号的 `anchor_room`**（`anchorPrefetchOnOpen` = 比开框前多 3 次，即 3 个已登录账号各一次）；② **跨账号管理**（第 3 条）：展开**非当前账号**（有直播间）的那一行即可管理它的房间，**不必切号**（`anchorAltPanelShown` / `anchorAltTitleFromRemote`，命令带 `account`）；③ 用**已开通**的当前账号点它：状态文本（`db-anchor-status`，在**标题输入框左边**，第 5 条）与开播 / 下播按钮**独占一行**（`db-anchor-live-row`）；标题输入框（`db-anchor-title`，初值 = 该直播间当前标题，与远端一字不差时「保存」禁用）；**两级联动分区选择**（`db-anchor-area-select`，父→子；选父落到它第一个子分区）；④ **改分区 = 独立写**（第 4 条）：改父 / 子任一级**立即**发 `anchor_area_set`（带 `account`），成功就地换分区名（`anchorAreaSetCalled`）；⑤ **开播成功后**下方**直接渲染** `db-anchor-config`：分区、推流地址（`db-anchor-rtmp-addr`）、推流码（`db-anchor-rtmp-code`），**长按该值即复制（不显示复制键）**；**下播 / 收起 / 关对话框**后这两行连同推流码一起**从 DOM 消失**（不是「看不见」）；⑥ 开播被**身份校验**挡住（`60043` / `60024`）时**弹出提示框** `db-anchor-gate-modal`（`FaceAuth` 给一枚「去完成人脸认证」入口走 `open_url`、`QrConfirm` 就地画离线二维码 `db-anchor-gate-qr`），提示「完成认证后再点一次开播」，且 **`QrConfirm` 的上游原话（code + msg）留在错误行**——引导不代替原话（`FaceAuth` 不留错误行红字，见 ⑩）；**不轮询、不自动重试**（等几秒 `anchor_live_set` 不会自己再发一次）；Esc 只关提示框、**不**连带关账号对话框；⑦ 读失败（凭据失效 / 风控）时只渲染错误行、**不静默**（`db-anchor-error` 是后端原话，`anchorReadErrorShown`），且该行按钮随之**不渲染**（`anchorNoToggleOnReadError`）；⑧ **ROOM_CHANGE 自动更新标题**（第 6 条）：上游推来 `danmubox://room` 且房间号命中当前直播间时，标题输入框就地换成新标题（`anchorTitleUpdatedByRoomChange`，限制：未连自己直播间收不到）。**开播 / 下播 / 改分区 / 改标题都是写操作**：只对 `account` 指定账号自己的直播间生效（`AGENT.md` §8 第 14–16 条 —— 失败即停、不重试）。⑨ **状态红绿**（issue202609242158 第 4 条）：`db-anchor-status` 按派生 `onAir`（`live_status` 1 / 2 = 在播）着红绿语义色（在播 = `--live-on`、未开播 = `--live-off`，与房间头状态点同一套令牌）—— 手工核对：未开播时状态字为红、开播后转绿且按钮文案翻成「下播」。⑩ **FaceAuth 无红字**（第 6 条）：开播被 `FaceAuth` 挡住时提示框里只有「去完成人脸认证」入口，**错误行不出现**「客户端老了」那类红字；`QrConfirm` 仍保留上游原话。⑪ **展开期静默刷新**（第 7 / 8 条）：展开管理区后每 30 秒静默重拉 `anchor_room`（收起 / 关对话框 / 换号即停），房管面板展开期每 60 秒重拉三块，开播 / 下播状态由乐观值 + 该轮询校正 —— 手工核对：展开后放着不动，标题 / 状态会被上游变化自动带上，**不必手动重开**。**本轮未跑无头冒烟**：`37-anchor-room.mjs` 未改（静态核对现有断言在新布局下仍成立），故按钮同排、红绿状态、FaceAuth 无红字、展开期轮询等新行为**暂无无头断言覆盖**，也未做三端真机核验 —— 无头冒烟同款断言见 `smoke/scenario/parts/37-anchor-room.mjs` 的 `anchorBlockRan` / `anchor*` |
...
| 写操作边界 | 只允许公开测试房间 `1`（5440）或当次明确指定的房间，一经指定不得更换，失败即停不重试；见 [`../AGENT.md`](../AGENT.md) §8 第 14–16 条。<br>**「我的直播间」（改标题 / 改分区 / 开播 / 下播，需求 §2.14）另有更窄的一条**：这类写操作**只对 `account` 指定账号自己的直播间**生效（“作用于哪个账号”在 `BiliAnchor::new_for(account)` 构造期决定，命令层只透传 `account`，端口签名不含账号）—— 目标房间由 `AnchorRoom::own()` 现取（`contract.md` §3），**上层不传房间号**，因此不存在「写到别人房间」的路径；失败即停，**不换房间 / 不换账号 / 不换参数重试**；上游非 0 code **原样带回、不赋语义**（`protocol.md` §18、附录 A66）；**唯一例外**是身份校验那两个码转成引导产出 `AnchorGate`（`protocol.md` §18.5）——引导**不触发任何自动重试**，认证完成后由用户自己再点一次开播 |
