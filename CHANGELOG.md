...
- **「我的直播间」补齐「开播人脸认证」的需求**（2026-09-22；需求 `REQUIREMENTS.md` §2.14，**本轮只补需求、不实现**）：
  ...
  口径与落点：端口 `AnchorRoom::go_live()` 在成功时返回 `StreamEndpoints`、被挡住时返回新的领域模型 `AnchorGate`（`docs/contract.md` §5：原始 `code` / `message` + `kind` ∈ `FaceAuth` / `QrConfirm` + `url` / `qr`）；认证页的拼装与 `data.qr` 的读取**只落在 `crates/danmubox-bili/src/anchor.rs`**，`core` 只承载那两个字符串、不拼装不解析（`AGENT.md` §8 第 3 条）。界面侧在错误行下面加一块引导 `db-anchor-gate`（`ui.md` §2.2.2）。
  ...
  **实现未做**：`crates/danmubox-bili/src/anchor.rs` 与前端 `AccountManager.tsx` 一行未动（用户本轮明确「先不实现，只补齐需求」），因此**无代码可测、本条无验证动作**。
