...
| 同一条只出现一次 | 回填与实时是**上游两条路**，同一条可能两边都有（`gethistory` 与 WS，或同一帧里压缩子包与明文子包各一份）。第二份在**进总线之前**就被丢掉（`danmubox-core` 的 `MessageSink::publish_with`，按 `uid + ts + 正文 + 表情` 认同一性）：界面与 `history_query` 快照看到的必须是同一份事实。**只对 `danmaku` 去重** —— 礼物 / 互动允许上游反复推同一条（连击、榜单刷新）。**合并（§8.4）不参与这件事**：真人重复发言每条各有各的上游 `ts`，照旧合并成 ×N；只有「同一条被送了两遍」才会命中这一层 |
...
6. 合并的前提是**上游真有 N 条**：同一条弹幕被上游送了两遍（回填 + 实时）在入列之前就被丢掉（§4.7），**不**参与这里的计数。`ui.merge_similar` 开关与窗口只影响「真人重复发言」怎么显示 —— 那是正则行为，不需要修。
