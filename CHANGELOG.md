...
- **界面收成一体系，并支持竖屏（手机比例）**（需求 §2；用户 2026-09-12：「要能缩到竖屏」「全局风格要一致」「每条弹幕的格式要优雅」）。
  ...
  - **窄屏（≤ 520px，主参考 390×844）**：同一套 DOM + 断点，不是两套结构。弹出面板（表情 / 短语 / 最近 / 筛选）、
    账号对话框、房管面板改为**自底部升起的 sheet**（内容可滚动、有明确的「关闭」入口、可点元素热区 ≥ 40px）；
  ...
  - 冒烟扩到**两个视口**（1440×900 与 390×844）各跑一遍同一份场景，新增排版与窄屏断言；两条宽屏专属口径
    （面板只挤列表 / 不遮最新一条）在窄屏显式跳过并写明理由——窄屏的面板是覆盖式 sheet。
