...
1. **同一台机上串行**：并发跑两份浏览器时，内存压力下系统会先杀掉 WebKit 的 WebContent 进程（`page.evaluate: Target crashed`），并**伪装成「某段场景必崩」**。崩溃可重试但上限 **1 次**：日志写明第几次尝试，重试必须**重跑整个场景**，两次都崩才报失败，且失败信息要说明「这是环境问题，不是断言不成立」。
2. **运行器自保**：每步（CDP 调用 / 连调试端口 / 导航 / 起浏览器）都有超时；退出（正常、断言失败、`SIGINT` / `SIGTERM` / `SIGHUP`）必收自己起的进程组，启动前清掉上次的孤儿（只杀自己标记的 `ppid==1` 遗留物）。浏览器候选依次自检：`CHROME_BIN` → omp 自带的 Chrome for Testing → 系统 Chrome，起一台先拍一张 `about:blank` 验它真会产帧。截图是证据不是断言，产不出帧时告警并跳过。
3. **改冒烟场景文件后有三道闸**（`AGENT.md` §9 的两道 + 本批补上的第三道）：`node --check` **不够**（模板字符串里的反引号不是语法错），还必须跑 `node smoke/run-headless.mjs --precheck` 真的求值一遍；`--precheck` 现在**同时**把构造出来的页面里那段内联 mock 脚本单独编译一次 —— 模板里的**反斜杠转义**是在求值时落地的（写一个换行转义会变成真换行、把字符串字面量当场掰断），前两道一个字符都看不见它，2026-09-15 漏过一次（两道全绿、起浏览器后以「场景未跑完（超时）」炸出来）。失败时第三道会带**场景文件行号**直接退出。
4. **夹具要能失败**：夹具从**真实载荷**派生（`smoke/fixtures/*.json`），**禁止手写 JSON**（`AGENT.md` §8 第 7 条）——手造样本曾把真故障藏住（P11）。样本尺寸必须与真图一致：头像用原图 **512×512**，表情用真实固有尺寸。
5. **Rust 改动另有一条**（`AGENT.md` §9）：必须真启动一次应用、确认存活 **≥ 10 秒**无 panic —— `cargo test` 与前端冒烟都挡不住「启动即崩」。
