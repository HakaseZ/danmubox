...
| P27 | 房间页头部重构：去掉「已连接 / verified」，改用**绿色=直播中 / 橙色=未开播**小圆点；返回键改**圆形左箭头**，**电池与「⋯」挪到上一排且同为圆形**；**直播间标题另起一排**；强化视觉效果（参考截图 `danmubox/打开哔哩哔哩继续观看.png`） | 用户 2026-09-12 对话（含参考截图） | 未做（进行中：HeaderPanelTabs） | 落点见对应组件与 `app.module.css`；深/浅两套都要成立 |
| P28 | 表情面板：去掉顶部「表情」标题与「关闭」按钮；左侧 tab 轨道**自身可上下滚动**；**高度降到 2 排表情格**（用户 2026-09-12 指出前两版都没理解此需求） | 用户 2026-09-12 对话（含参考截图） | 未做（进行中：HeaderPanelTabs） | 落点见对应组件与 `app.module.css`；深/浅两套都要成立 |
| P29 | 主页不显示弹幕页的 tab 条（与「已连接房间」功能重复） | 用户 2026-09-12 对话（含参考截图） | 未做（进行中：HeaderPanelTabs） | 落点见对应组件与 `app.module.css`；深/浅两套都要成立 |
| P30 | 主页内容未居中：右边距看起来比左边宽（应为左右相等） | 用户 2026-09-12 对话 | 未做（进行中：HeaderPanelTabs） | 静态定位到 `.listPage`（padding 对称 + max-width + margin auto + overflow-y auto）与 `index.css` 的占宽滚动条；修法与改前/改后左右距离数字见该票报告 |
| P31 | 发送失败不要在页面最下方出常驻提示；改为**浮动提示 + 渐隐消失**，且提示期间**不得遮挡或阻断弹幕区的滚动** | 用户 2026-09-12 对话 | 未做（进行中：HeaderPanelTabs，本批最后一项） | 落点 `Composer.tsx` + `app.module.css`；可验形式：浮层矩形不与弹幕列表区域相交 + `pointer-events: none` + 渐隐后移除 |

**产品需求小计：31 条**（已做 22 / 部分 1 / 未做 8）。
