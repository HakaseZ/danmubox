...
另一条需求（REQUIREMENTS.md）是「B 站 API 不可控，逆向改动不能影响核心业务」。这要求 `core` 与具体上游实现之间也有明确边界：`core` 只定义端口与领域模型，实现全部在 `danmubox-bili`（见 [`0004-upstream-isolation.md`](0004-upstream-isolation.md)）。
...
| 允许依赖 | 通用基础设施：异步运行时、序列化、日志与错误处理、本地文件读写；**不含**任何 B 站专用库或字段 |
...
- 上游隔离有明确的落点：`core` 里不放 B 站事实，逆向时改动被限制在 `danmubox-bili`。
...
| `core` 被反向污染 | 为图方便在 `core` 里 `use tauri::...` 或直接引入 B 站库 | 依赖方向纳入 CI 检查；`core` 的 `Cargo.toml` 不出现 `tauri`，也不出现上游专用依赖 |
