...
| `MessageRow.tsx` | 单行渲染：时间列（受 `ui.show_timestamp` 门控）、头像列、身份行（昵称 + 徽标）、正文（含 `@` 高亮）、失败 / 未确认标记、行右键入口（§4.1 / §4.2 / §4.4 / §4.5） |
...
一行固定为 **`[时间戳列][头像][正文块：身份行（用户名 + 身份牌）/ 正文行][连击计数（仅礼物连击行）]`**（用户 2026-09-12：重做每条弹幕的 DOM；用户 2026-09-13：正文块改成**上下两行**）。
...
| 正文拿到整行宽度 | 正文的可用宽度 = 行宽 − 时间戳列 − 头像列，**不再被身份簇挤走**，两种视口都 ≥ 视口的 50%。旧的 `--identity-max-w`（14em / 窄屏 10em）与 `grid-template-columns` 是**给两列分宽度**才存在的，随布局一起删掉（改前 / 改后的逐项度量见 [`CHANGELOG.md`](../CHANGELOG.md)）。冒烟按 `fixtureTextBodyKeepsHalfViewport`（正文 ≥ 视口一半）、`fixtureTextBodyFillsBlock`（正文吃满正文块）、`fixtureBodyAlignedWithName`（正文左边缘 = 用户名左边缘）断言 |
