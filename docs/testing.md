...
| 系统栏避让（A-2 的前半） | 提交 `32dcefc` 前后逐值实测：顶栏文本 `y=68..116` → `196..244`、主题按钮 `74..114` → `202..242`、房间页输入区白底 `2399` → `2338`（手势栏上沿 2337），状态栏图标仍在 47..80、互不相交；`am start -W` COLD `TotalTime` 515ms、logcat 无 FATAL（数字与做法见 [`operations.md`](operations.md) §5.3） |
