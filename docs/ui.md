...
**房管三项必须做权限前置**：身份取自 `RoomSession.is_admin`（IPC `room_session`，契约 §7），不是房管就置灰并写明原因——绝不做成「点了再看上游权限错误码」。
...
房管能力分两个入口：**行右键菜单**（对某个具体的人）与**房管面板**（名单与屏蔽词的增删查）。权限前置统一取自 `RoomSession.is_admin`（IPC `room_session`，契约 §7）：**拿不到身份 = 按无权限渲染**，绝不先放行再等服务端报错。**房管面板的入口只在 `is_admin === true` 时存在**（需求 §2.10；判据与官方 web 一致：`protocol.md` A38 的 `getInfoByUser` → `data.badge.is_room_admin || admin_level > 0`）——不是置灰、不是点开再看上游报错。
...
三枚开关（含自动消失，§4.8）**都只是显示层的派生**：原始消息始终留在会话缓冲里，关掉开关即复原。
...
> 悬停提示（`title`）在这些开关与其他同类控件上只写**字段名**（控件名，如「时间戳」「互动消息自动消失」），不写操作教学、不写上游字段名与协议章节号。**例外是本身没有可见文字的控件**：主题按钮的 `title` / `aria-label` 必须带上当前档与下一档（§8.3）。
