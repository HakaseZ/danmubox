...
| 房管面板 | 与弹出面板同层（输入区上方），由房间头 `⋯` 菜单开合（**该菜单项只在 `RoomSession.is_admin === true` 时存在**，§4.9）；展开时同样只挤压弹幕列表（§4.9） |
...
| 房管面板 | 房间头 `⋯` 菜单 →「房管面板」（**仅 `is_admin === true` 时该菜单项才渲染**；非房管与身份未取到时菜单里没有这一项） | 三块：禁言名单、黑名单、屏蔽词；列表 + 增删 |
...
- **面板内的三块列表照常请求上游，拒绝原因原样展示**：上游的拒绝原样展示为 `code` + `message`（不翻译、不猜语义），三块的错误各自独立显示、不互相清空。由于入口本身只对房管出现（上一节），这一档只覆盖「面板打开期间身份被撤销 / 重取不到」：那时面板随之收起（`isAdmin` 不为 `true` 时不再保留开着的面板），面板内若还画着只读兜底提示，文案也按「房管身份已失效」写，不写成「无权限也允许打开」。
...
| 图标 | **下箭头**（用户 2026-09-13 #1：「回到最新」的图标改为下箭头 = 返回键旋转 90°）。与房间头的返回键**同源几何**——§3.1 的矢量规范四条（同一 `viewBox="0 0 24 24"`、同一 `stroke-width: 1.75`、round 线帽 / 线接、墨迹居中于 (12,12)、主轴 16 单位）原样成立，只把返回键 `M15 4.875 9 12l6 7.125` 的几何绕 (12,12) 转 -90°（朝下）：`M4.875 9 12 15l7.125-6`。**直接写转好的 path，不挂 CSS `rotate()`**（省一层 transform，描边宽度与墨迹居中不必再跟变换打架）。盒子是 1.5em，于是 24 单位盒里的 16 单位墨迹正好 1em 高、与按钮文字同档，并随字号档一起缩放。箭头排在文案左侧、`aria-hidden`：它是装饰，按钮的可访问名仍是「回到最新」 |
