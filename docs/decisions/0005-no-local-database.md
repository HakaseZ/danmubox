...
同时，B 站本身不提供弹幕历史回放接口，客户端拿不到「这个房间过去的弹幕」，所谓「历史」只可能是本地这段时间收到的消息。这一点是接受的取舍，而不是待补的功能。
...
| 容量 | 会话内上限 **5000 条**（偏好键 `history.buffer_rows` 可覆盖），超出丢弃最旧 |
| 进程退出 | 缓冲随进程消失，不做任何落盘 |
...
偏好键 `history.buffer_rows` 与其余取值以 [`../contract.md`](../contract.md) §8 为准；缓冲字段形状（`Message`）以同文 §5 为准。
...
- 内存占用有确定上界：单房间最多 5000 条，随房间数线性增长且可预估。
...
- 缓冲写满后丢最旧：长时间停留在一个高流量房间时，用户向上回滚能看到的窗口有限。
- 多房间同时连接时，内存占用是「5000 条 × 房间数」，房间开得多时需要留意（自用场景房间数很小）。
...
| 风险 | 说明 | 缓解 |
|---|---|---|
| 内存占用超预期 | 大房间 + 多房间并行，环形缓冲总量上升 | 上限由 `history.buffer_rows` 控制；房间数与缓冲条数都可由用户调低；[`../operations.md`](../operations.md) 给出观测方式 |
