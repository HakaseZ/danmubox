...
> 定位：ac站直播间弹幕客户端（自用不发布），面向三端的聊天框式弹幕工具。
...
「弹幕框」复刻的是 ac站官方直播间的**聊天框**：一个按时间流动的文本消息列表。
...
- 登录：扫码（唯一入口，手机 ac站 App 扫码）与游客态；凭据文件 `config.toml` 由程序管理，没有导入界面
- 单次房内会话的内存弹幕缓冲（按 `kind` 六档，各档上限见 §9）+ 房间内「刷新」手动重连
...
四层单向依赖：`danmubox-bili`（唯一接触 ac站的适配器）→ `danmubox-core`（领域模型 + 端口 + 事件总线 + 会话编排 + 本地文件）；
...
| [`docs/protocol.md`](docs/protocol.md) | ac站弹幕协议：包头、op、protover、认证与心跳包（WS + HTTP）、子包拆分、重连状态机；**附录 A 是全仓唯一的「待实测校准」表** | 实现者 |
...
| 项 | 说明 |
|---|---|
| 网络出口 | 仅连接 ac站直播相关域名（WS 长连与 REST 接口） |
| 凭据文件 | `config.toml`，**明文 TOML**，权限 **0600**，位于本机数据目录；**由程序管理**（界面与 CLI 都不提供 Cookie 导入入口） |
...
- 本项目是**非官方**第三方客户端，与ac站及其关联公司**无任何隶属、合作或背书关系**。
- ac站名称与商标归其权利人所有，本项目仅作描述性使用。
- 本项目自用、不发布、不商用、不提供任何形式的服务；使用者需自行遵守 ac站用户协议与当地法律法规。
- 协议实现依据公开可见的客户端行为整理，ac站可能随时调整协议；因此产生的功能失效由使用者自行承担。
