...
- 会话内保留**按 `kind` 分档**的内存缓冲（issue 2609171849 第 3 条）：每档各按自己的上限丢最旧，进程退出即丢。
...
- **礼物按金额分级保留**（`Message.amount` 金瓜子，口径见 §5「金额单位」）：礼物档的额度再按
  **10% / 40% / 50%** 切给低 / 中 / 高三档，**价值越高留得越多**。三档各自丢最旧，
  因此同样的条数在高档覆盖的时间轴远长于低档（高档礼物本来就到得少）。
  三档条数之和**恒等于**礼物档额度（取整的余数给高档）；每档至少 1 条。
  ...
  档位是**本地保留策略**的取舍，**不对上游礼物价位作任何断言**：它只决定「同一条道里留多少条」。
  只有 `kind = "gift"` 参与分档；SC（金额以元计）与大航海各占自己那一档，不分档。
...
| `amount` | i64 | 金额：**礼物与大航海是金瓜子数，SC 是元**，非交易类为 `0`。界面展示一律按**元**（换算见本节「金额单位」）。**大航海取实付**：同一笔开通上游发两条载荷，只有播报（`USER_TOAST_MSG.price`）是实付、购买事件的 `price` 是标价**不计**，且两载荷只出一条（`protocol.md` §10.6 / §12.3） |
...
>
> **大航海的取值口径（2026-09-17 实测，字段级依据见 `protocol.md` §10.6）**：同一笔开通上游会发
> `GUARD_BUY`（购买事件，`price` = **标价**：舰长 198 元）与 `USER_TOAST_MSG`（播报，`price` = **实付**：
> 舰长 138 / 168 元）两条载荷。`amount` 取**播报的实付**，标价不计；两条合并成一条播报（§10.6 / §12.3），
> 因此同一笔在界面统计里只算一次。只有购买事件、窗口内没等到播报时 `amount = 0`（不拿标价冒充实付）。
...
| `ui.interact_auto_hide` | boolean | `true` | 互动/进场消息显示一会儿后自动消失（`false` = 常驻）。消失**只是显示层不画**（判据 `ts + INTERACT_AUTO_HIDE_MS`）：消息仍留在会话缓冲里（§4.3），关掉这枚键先前消失的那些行**原样回来**——「自动消失」不许丢内容，见 [`ui.md`](ui.md) §4.8 |
...
| `history.buffer_rows_danmaku` | integer | `5000` | **弹幕档**上限（每房间、每次会话） |
| `history.buffer_rows_gift` | integer | `2000` | **礼物档**上限；档内再按金额切低 10% / 中 40% / 高 50%（§4.3） |
| `history.buffer_rows_superchat` | integer | `500` | **SC 档**上限 |
| `history.buffer_rows_guard` | integer | `200` | **大航海档**上限 |
| `history.buffer_rows_interact` | integer | `300` | **互动 / 进场档**上限（刻意小于弹幕档） |
| `history.buffer_rows_system` | integer | `200` | **系统通知档**上限（刻意小于弹幕档） |
...
| **两个区域** | 「两个区域」= **弹幕区**与**礼物栏**（共享上下分区的两栏，§8 `ui.gift_pane_on_top`；礼物类消息按 `ui.gift_in_danmaku` / `ui.gift_panel` 落进这两栏，默认两处都有）。折叠在**两处各生效一次**；剔除统计的**统计面只有礼物栏折叠头那一处**（弹幕区没有统计面，因此这枚键在那一栏什么都不改 —— 不编造第二处统计）。两处共用同一枚 `isCheapGift` 判据与同一个桶实现（`filtering.collapseCheapGiftRows` / `giftStatRows`） |
...
| 单次会话内保留弹幕 | §4.3 |
| **消息按 `kind` 分档缓存 + 礼物按金额分级保留**（issue 2609171849 第 3 条） | §4.3（分档表与礼物三档）、§8 的六枚 `history.buffer_rows_*` |
