...
| F-08.2 | 低价礼物（≤ 0.1 元）的折叠与统计剔除 | 勾上「辅助功能」里的「折叠低价礼物」/「剔除低价礼物统计」，礼物栏里放 0.09 / 0.10 / 0.11 元与没给价（`amount = 0`）四条礼物 | 折叠只改礼物栏**条目数**（两条低价合成一条，`×N` 与金额是整桶合计），折叠头的统计逐字不变；剔除只改**折叠头**的「礼物 / SC（N）」与三组明细，礼物栏条目与弹幕流照旧；边界：0.09 / 0.10 算低价、0.11 与 0 不算；SC 与大航海不受两枚开关影响；两枚默认都关且持久化（`ui.md` §5.3、`contract.md` §8；无头冒烟同款断言见 `smoke/room-page.mjs` 的 `cheapGift*`） |
...
| `swapDoesNotEatNextTap`（长按换位松手后，紧接着的那一次点击照旧生效 —— 旧实现那枚 600ms 全局捕获定时器会把它吃掉） | 换位松手后紧接着按礼物折叠头（`pointerdown` → `pointerup` → `click` 三步齐全），折叠头**没有开合**。同段其余 14 条 swap 断言全绿，只有这一条红 | **未修、不在本票范围**（属 2609162056 的 splitter / swap 段，本批跑完整冒烟时首次在浏览器里执行）：chromium 四组合逐位相同 |
| 低价礼物两枚开关那一段的 11 条（`cheapGiftFreshRoomTab` / `cheapGiftFreshRoomActive` / `cheapGiftThreeRowsSeparate` / `cheapGiftFoldMergesRows` / `cheapGiftBucketRow` / `cheapGiftNonCheapRowStays` / `cheapGiftExcludeKeepsRowCount` / `cheapGiftExcludeKeepsDisplay` / `cheapGiftExcludeChangesStats` / `cheapGiftZeroPriceNotCheap` / `cheapGiftScGuardUntouched`） | 入口先红：`cheapGiftFreshRoomTab` / `cheapGiftFreshRoomActive` 为假（那一段要的**空会话新房间**没被切过去），后面 9 条因此全在同一套空礼物栏上量 —— `cheapGiftRowsBeforeFold` = 0、`cheapGiftSummaryBeforeFold` = 与 `cheapGiftSummaryBoundary` = 「本场暂无礼物」、折叠头文本「礼物 / SC（0）… 本场还没有礼物」。夹具那一侧没问题：同段的 `cheapGiftSwitchesDefaultOff` / `cheapGiftFoldToggled` / `cheapGiftFoldPersists` / `cheapGiftExcludeToggled` 都是真 | **未修、不在本票范围**（属 2609162056 的低价礼物段）：chromium 四组合逐位相同 |
