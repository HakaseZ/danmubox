...
| P26 | 整体设计风格模仿 WhatsApp：**全应用一次换完**，深浅两套配色并**跟随系统**；只取设计语言（色板/圆角/间距/字体层级/顶栏与输入栏形态），**不照搬气泡结构**（弹幕是多人流水） | 用户 2026-09-12 对话（选「全应用一次换」+「深浅两套跟随系统」；无截图，按设计规则做） | 已做 | 令牌层补齐两套语义槽（`app.module.css` 的 `:root` / `:root[data-theme="light"]`，逐条标来源与对比度）、`index.css` 元素默认改胶囊 + `accent-color`；主题开关在筛选面板「显示」块（`db-pref-theme`，写回 `ui.theme`），并补系统外观变化的实时订阅与首帧前预设；**不照搬**气泡尾巴 / 已读回执 / 未读徽章布局，理由逐条写在 `docs/ui.md` §8.3。冒烟新增主题维度：`SMOKE_THEMES=dark,light` × 2 视口 × 2 引擎（Chromium / WebKit）**全部退出码 0**，每引擎 1330 项快照 / 40 张截图（两引擎共 80 张；原基线 629 项 / 20 张，新增断言 18 条 × 2 视口 × 2 主题）；两档实测对比度：深色正文 15.79:1 / 次级 6.10:1，浅色正文 14.58:1 / 次级 5.06:1。落点：`docs/ui.md` §8.3 / §9.2 / §15、`CHANGELOG` Unreleased Added；分支 `feat/whatsapp-restyle`，提交 `6b2b8f2` |

| P27 | 弹幕页：不要「已连接 verified」，靠绿 / 橙小点区分；返回改**圆形左箭头**，电池与 `⋯` 也做成同形状圆形并挪到上一排；直播间标题另起一排；顶栏做成**半透明深色层次**。表情面板：顶部的「表情」与「关闭」不要，左侧 tab 轨道自己上下滚，高度降到 **2 排表情包**。主页不显示弹幕页的标签条（与「已连接房间」重复）；主页「不对称」实为**高度问题**：**高度缩到出现滚动条之后，元素右侧回缩**（原描述「右边距比左边宽」是误判，用户 2026-09-12 亲自更正）；发送失败不要在最下出常驻提示，改**浮窗 + 渐隐**且过程不挡弹幕；自己发的弹幕**不需要高亮**（`rowOwn` 系主 agent 夹带的替代做法，已整条删除）。 | 用户 2026-09-12 对话（四条反馈 + 追加两条澄清 + 一条纠正） | 已做 | `app.module.css`（`.roomHeader` 两排 + `.ctlRound` / `.liveDot`、`.picker`/`.pickerBig` 两行算式、`.listPage` 的 `scrollbar-gutter`）；`index.css` **删掉自定义滚动条**（它把 macOS 覆盖式滚动条换成占宽的经典条，同时造成「右侧回缩」与「窗口右边缘拖不动」）；`RoomView.tsx` / `Composer.tsx` / `App.tsx`；冒烟新增断言 `headerControlsAllRound` / `liveDotMatchesStatus` / `liveDotFollowsStatus` / `panelEmoteHeaderGone` / `panelEmoteGridTwoCommonRows` / `panelEmoteRailScrollable` / `listPageRightEdgeStable` / `sendFailToastClearsList` 等 |

**产品需求小计：27 条**（已做 23 / 部分 1 / 未做 3）。
