...
- 会话内允许向上回滚查看（`history_query`）；ac站无历史回放接口，本设计不做跨会话历史，也没有向前翻取更早内容的入口。
- **「刷新」在两档下的界面口径不同，判据是 `rooms[].connected`**（= 外壳里还有没有这个房间的运行时）：
  ...
  - 判据**不能**取 `status[room_id]`：长连接掉线、准备退避重连时推的也是 `disconnected`（`danmubox-bili/src/ws.rs`），那一次会话还在、缓冲还在，按「重新进房」处理会白白清掉用户正看着的弹幕。
  - **机制级验证**：单测 `node --test src/session-messages.test.ts`（在 `apps/desktop/ui` 下）：会话重建后新会话的弹幕照旧上屏（修前那一路会一条都进不来）、同一条从回填与实时两条路来只画一行、会话还在时刷新不动列表、本地待确认行活过换代、快照没覆盖到的行不被抹掉。
...
| 会话已被「断开连接」结束 | 当场**重建一次会话**（等价于重新进房：缓冲从空开始），界面**同口径换成新会话的快照**（先摘掉上一个会话的行、建连后整批落地一次 `history_query`，只留本地待确认行；§2.4）。不换的话新会话的 `local_id` 从 1 重新编号，新消息会被「只收更新的号」那条静默丢掉。断连之后这颗键不得是死键 —— 否则用户只能返回列表再进来。这是「断连后无法直接重连」那条反馈的落点 |
...
| 口径 | 规则 |
|---|---|
| 身份行在上、正文行在下 | 正文块是**上下两行**（参考图 `打开ac站继续观看.png` 的口径）：第一行「用户名 + 身份牌」，第二行正文**另起一行**、左起点与用户名对齐。两块都是块级，所以正文必然落在自己的行上（结构本身给的悬挂缩进，不需要 `padding-left` / 负 `text-indent`）。⚠ 正文块用 **`display: flow-root`** 而不是 `block`：块级大表情带一个负的 `margin-block`，普通块会与它**边距折叠**、把整个正文块上提 1.05px 骑到身份行上（2026-09-13 改块级当天冒烟实测 bodyTop 比身份行底边高 1.1px；旧版是网格项，网格项的边距不折叠，所以看不出这毛病）。用户 2026-09-13：「长文本弹幕自动换行还是没有实现好」——根因就是旧的**左右两列**排法：窄屏 360 下身份簇吃掉正文块的 56%（旧版 140px / 500px 上限口径下仍有 4 成），正文列只剩 112–165px，长文本自然折得又窄又碎 |
...
- 自动消失的实现（issue 2609171849 第 5 条改）：行按 8 秒跑一段淡出动画，**到点只是不再画这一行** —— 判据 `filtering.interactAutoHidden`（`ts + INTERACT_AUTO_HIDE_MS <= now`）在**显示层**的派生里（`filtering.toDisplayRows`），与动画共用同一个常量（`types.ts` 的 `INTERACT_AUTO_HIDE_MS`），两者不会错位。`store` 那批定时器只做一件事：到点把 `interactTick` 挪一格，叫醒派生重算。
  **改前是 `messages.filter(...)` 把消息从列表里摘掉** —— 那是真的丢内容：`ui.interact_auto_hide` 拨回 `false` 只能让**之后**来的行常驻，早先消失的永远回不来。现在的口径是「消失」属于显示层：消息一直在会话缓冲里（受契约 §4.3 的上限约束），关掉开关同一帧就回来。**离开房间仍要清掉这些定时器**（`store` 的 `clearRoomTimers`），否则残留的定时器会把另一个房间的画面算到别的时刻上。
...
| 冒烟按 | `cheapGift*` 一组：两枚开关默认关且可持久化（点开 → 翻偏好 → 再点回）、折叠开启后礼物栏**条目数真的变少**且合并行的 `×N` / 金额是整桶合计、剔除开启后**汇总金额与总条数真的变少**而礼物栏条目数不变、四个边界值各自归属、SC 与大航海在这两枚开关四种组合下都不变。`switchScope*` 一组（issue 2609171849 第 5 条）：**两个区域各量一次**折叠（弹幕区的桶行带 `×2`、礼物栏条目 3 → 2）、关掉后两处逐条复原（顺序 / 条数 / 金额 / 统计串）、剔除只动统计不动任何一处的行、自动消失到点不画而关掉开关原样回来 |
...
| 徽标底色 | `--gold`、`--neutral`、`--guard-1…3`、`--sc-1…5`、`--block-platform`、`--block-room`、`--badge-fg`、`--badge-lift`、`--badge-border` | 见 `:root` | 粉丝牌**不吃**这些：它的真彩色来自上游（§4.2），令牌只兜底；`--sc-1…5` 是**醒目留言卡片的五档底色**（本批起被消费，改前全仓没人用它）：深色基座 `#2f6fd0` / `#1e9e8a` / `#c9a227` / `#d1603d` / `#c0392b`，来源标注【B+】——**本项目自己的设计令牌**，与 ac站网页端 SC 卡片的逐档比对**没做过**（`protocol.md` 附录 A 的 A.2「SC 卡片配色档位边界」仍待校准）；档位边界（100 / 500 / 1000 / 2000 元）同样是本地取值，依据见 §4.1。卡片实际用的是「该档色 22% 混 `--bg-elevated`」的底色 + 该档实色描边 |
...
| 维护约定 | 场景 = `smoke/scenario/parts/**` 里的**页内脚本原文**（`00-mock` / `10-harness` / `20…36` 各主题块 / `90-epilogue`），由 `smoke/room-page.mjs` 按文件名升序拼接；**加一条断言**就改对应主题块（新起一块才要在组装器的 `BLOCKS` 里登记一行，多登记少登记都会在构造页面时当场抛错），写法与准入看 [`testing.md`](testing.md) §9.1 / §9.3。片段是原文拼接，反引号 / 反斜杠 / `${` 都按原样进页面 —— **不要**再把它塞回模板字符串 |
