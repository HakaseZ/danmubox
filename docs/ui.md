...
**被吞（`"f"` / `"k"`）没有专门的行级标记**：`store.send` 对非 `ok` 一律走同一套「本地行标成发送失败（§4.4）+ §6.5.1 浮动提示」，颜色不分档，具体原因由浮片文案里的 `detail` 带上游原话 + code 表达；判定来源见 `protocol.md` §11.2。更早那版设计过的「红 / 黄删除线 + 角标」两档从未实现，沿革见 [`CHANGELOG.md`](../CHANGELOG.md)。
