...
| P27 | 房间页头部重构：去掉「已连接 / verified」，改用**绿色=直播中 / 橙色=未开播**小圆点；返回键改**圆形左箭头**，**电池与「⋯」挪到上一排且同为圆形**；**直播间标题另起一排**；强化视觉效果（参考截图 `danmubox/打开哔哩哔哩继续观看.png`） | 用户 2026-09-12 对话（含参考截图） | 已做（待用户复测） | 落点见对应组件与 `app.module.css`；深/浅两套都要成立 |
