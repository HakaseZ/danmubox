...
| `config` | `config.toml` 凭据存储：`Profile` 七字段、`active_profile`、`ConfigStore` 的加载与原子替换写、手写 `Debug` 遮蔽 | 不发网络请求；不判定凭据是否有效（→ `bili::auth`） |
| `prefs` | `prefs.json` 偏好白名单：契约 §8 的全部键（23 项 `SPECS`，逐键由 `spec_table_matches_contract_keys` 直接读契约比对）、读时与默认值合并、写时未知键 / 非法值报 `BAD_REQUEST` | 不存凭据；不含展示与过滤逻辑 |
| `error` | `DanmuboxError` 枚举与 `Result`，进程内错误归一化 | 不定义 IPC 错误码集合（见 `ipc.md` §2） |

### 2.2 `danmubox-bili` 模块划分

全部十六个文件（`crates/danmubox-bili/src/`，含 crate 根 `lib.rs`）：

| 模块 | 职责 | 实现的端口 |
|---|---|---|
| `lib` | crate 根：模块声明与公共再导出（`BiliLive`、`BiliAuth`、`BiliSender`、退避函数等），是唯一允许 `bili` 之外代码引用的装配面 | —（装配面） |
...
| `cmd` | 上游 `cmd` → 领域 `kind` 的归一化（取值表见 `protocol.md` §10） | —（被 `ws` 使用） |
...
| `RoomAdmin` | 房管：禁言 / 黑名单 / 屏蔽词 | `room_id`（写操作另带 uid / 词）→ 名单数组或写操作结果 | `bili::admin` |
| `RoomCatalog` | 关注列表与直播状态 | 无输入 → `FollowedRoom[]`（`live_status == 1` 置顶由实现内的 `core::model::sort_followed` 完成） | `bili::follow` |
...
graph TD
  ...
  N2 --> B2
  N2 --> BUS
  BUS --> S1["Tauri 事件桥<br/>danmubox://message / room / room_stats / status / session / send / log"]
  BUS --> S2["CLI 前台打印 / NDJSON"]
```

事件路径是**一份产出、两处消费**：完整数据流见 §4.5，去重为什么必须做在两处见 §4.7。

| 任务 | 数量 | 生命周期 | 失败影响 |
|---|---|---|---|
| 房内会话（driver / collector / 身份三个任务） | 每连接房间 1 个 | `rooms_connect` 创建；`rooms_disconnect` / `rooms_remove` / 进程退出时 `close()` 取消 | 只影响该房间；其它房间继续工作 |
| 心跳定时器（WS 30s + HTTP 60s） | 每个房内会话各 1 组 | 随会话 | 心跳写失败立即判定连接不可用并进入重连 |
...
core 只读写两个本地文件，路径解析在 `core::paths`（`paths.rs:8`）：macOS `~/Library/Application Support/danmubox`；Windows `%APPDATA%\danmubox`；其余平台 `~/.local/share/danmubox`；可用环境变量 `DANMUBOX_HOME` 覆盖整个数据目录。
...
1. 两个文件分别只由 `core::config`（`config.rs`）与 `core::prefs`（`prefs.rs`）落地，路径来自 `core::paths`；凭据字段的语义（哪些字段必填、何时判定失效）由 `bili::auth` 的 `AuthProvider` 实现决定。
...
统一错误类型在 `core::error`；IPC 错误对象与错误码集合见 `ipc.md` §2（本文不重复定义）。与连接相关的重试不走错误码，走 `bili::ws` 的 driver 重连状态机（§4.4）。
...
| 重连 | 重连前重新走 `getDanmuInfo` 取票据与 host 列表，不复用过期参数（`bili/http.rs:29`） |
| 单包解压超限 | 丢弃该包并计数（上限 16 MiB，防解压炸弹），不打断连接 |
| 上游无法识别的推送 | 不打断连接；`SYSTEM_CMDS` 之外的丢弃并计入 `unknown_cmd`（`bili/cmd.rs:136`） |
...
| 目标 | stdout / stderr；同一条日志经 256 容量的广播通道（`apps/desktop/src-tauri/src/lib.rs:965`）桥接成 Tauri 事件 `danmubox://log`（`lib.rs:1014`），前端只保留最近 200 行（`store.ts:31` 的 `LOG_CAP`） |
| 结构 | 时间戳、级别、target、span 路径、消息、结构化字段 |
| 级别约定 | `error` 需人工介入；`warn` 可自恢复（重连、丢包）；`info` 生命周期事件；`debug` 包级明细（解包长度、op、protover）；`trace` 逐条消息 |

### 9.2 tracing span 设计

下列 span 名与字段是**命名口径**：当前代码只发 `tracing` 事件（`debug!` / `warn!` 等），尚未引入 span 宏；新增带上下文的日志时按本表命名，避免出现第二套名字。
