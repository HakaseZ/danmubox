...
- **Android 端开工：可出包、可安装、可启动**（用户 2026-09-15；分支 `feat/android-mobile`，提交 `0e1bde6` / `40152e0` / `a71c5a3` / `62ea667`）。交付物三块：
  ...
  **已知问题（已发现，修复中）**：targetSdk 36 强制 edge-to-edge 而界面未处理窗口 inset —— 顶栏落进状态栏带（StatusBar frame=[0,0][1080,128]、刘海 top inset=128），右上主题按钮与电池图标重叠；本分支另有修复票在跑，是否已修以分支 `git log` 为准。
