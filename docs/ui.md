...
| 合并计数 | 礼物行始终渲染 `×N`（连击折叠后的 `count`，未折叠时为 `×1`）；其余 `kind` 不再出现 `×N` —— 「相似消息合并」已删除（§8.4），`count > 1` 只可能来自礼物连击。`×N` 是正文**行内**末尾一格：跟在最后一行文字后面，不另占一行、不另开一栏 |
...
| 其余 `kind` | **不折叠**，一条消息一行。原先的「相似消息合并」（同 uid + 同正文 + 时间窗，偏好键 `ui.merge_similar` / `ui.merge_window_ms`）由用户 2026-09-13 判定**不是他要的功能、也不必要**，已整条删除（见 `docs/requests.md` P49）；偏好键从契约 §8 与 `prefs.json` 一并移除 |
