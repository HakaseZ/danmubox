...
| A5 | `Message.is_admin`（房管标记） | 发送者是否房管的判定字段名与取值形态（布尔 / 等级 / 位标志） | 同上，需一名房管账号发言样本 | **部分解决（2026-09-12）**：**SC 载荷自带 `user_info.manager`**（实测样本为 0）——SC 的房管标记因此有据；但 `DANMU_MSG` 用的经典槽位 `info[2][2]` 仍缺正向样本 | `cmd.rs`、§10.1 |
